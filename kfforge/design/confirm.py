"""kfforge.design.confirm -- the approval protocol.
...
pure/never-mutate convention the rest of kfforge (graph.py, pages.py, nav.py) already follows.
Duck-typed against kfforge.design.AppSpecLike throughout, via `kfforge.design.diagram._seq()`/
`_seq_replace()` for every dimension that kfforge.intake.schema wraps in its own container
(`Stages`/`Routing`/`ReworkLoops`/`TestCases`) -- never imports kfforge.intake (see the package
docstring for why).
"""
...
from .mockup import design_bundle_html

if TYPE_CHECKING:
    from . import AppSpecLike
...
def _freeze(value: Any) -> Any:
    ...
    Dataclasses (both kfforge.intake's real AppSpec -- including every dimension WRAPPER, e.g.
    ...
    kfforge.intake's actual shape, or even of this package's own Protocol attribute names, since
...
def _required_field_questions(spec: AppSpecLike) -> list[str]:
    return [
        ...
        for f in _seq(spec.data_model.fields) if f.required
    ]
...
# revision key because a terminal state is DERIVED (kfforge.design.diagram._terminal_stages),
...
# customer disputes a terminal state should re-open dimension 3/4 with kfforge.intake, not expect
...
def _mapping_like(original: Any, new_dict: dict) -> Any:
    ...
    mapping: kfforge.intake's real DecisionPoint stores it as a tuple of (key, value) pairs (this
    package's own frozen-collections convention -- see kfforge.intake.schema's module docstring),
    ...
    field's collection type from what the rest of kfforge.intake expects."""
    if isinstance(original, tuple):
        return tuple(new_dict.items())
    return new_dict


def apply_revisions(spec: AppSpecLike, revisions: Mapping[str, str]) -> AppSpecLike:
    ...
    kfforge.intake's AppSpec and the protocol-shaped stubs in tests/test_design.py are.
    ...
    StepFill.stage) so a renamed spec still round-trips through kfforge.intake's own
...
def _apply_one_revision(spec: AppSpecLike, key: str, value: str) -> AppSpecLike:
    ...
    if len(parts) == 4 and parts[0] == "loop" and parts[3] in ("gate_field", "from_stage", "to_stage"):
...
def _rename_stage(spec: AppSpecLike, old_name: str, new_name: str) -> AppSpecLike:
    """Cascades a stage rename into every place kfforge.intake.schema stores a stage name as a
    plain string reference. Proven necessary, not speculative: a first review round built a real
    AppSpec, renamed one stage via this function, and fed the result to kfforge.intake.compile.
    ...
    new_stages = [
        dataclasses.replace(st, name=new_name) if st.name == old_name else st
        for st in stages_seq
    ]
    ...
    new_fields = [_rename_field_stage_and_section(f, old_name, new_name)
    ...
    for case in _seq(result.test_cases):
        ...
        new_cases.append(dataclasses.replace(
            case,
...
def _revise_routing_target(spec: AppSpecLike, at_stage: str, option: str, new_target: str) -> AppSpecLike:
...
def _revise_routing_option(spec: AppSpecLike, at_stage: str, old_option: str, new_option: str) -> AppSpecLike:
...
def _revise_loop_attr(spec: AppSpecLike, from_stage: str, to_stage: str, attr: str, value: str) -> AppSpecLike:
    ...
    new_loops = [
        ...
        if lp.from_stage == from_stage and lp.to_stage == to_stage else lp
        for lp in loops_seq
    ]
...
def _revise_field_required(spec: AppSpecLike, stage: str, name: str, value: str) -> AppSpecLike:
    ...
    new_dm = dataclasses.replace(spec.data_model, fields=_seq_replace(spec.data_model.fields, new_fields))
    return dataclasses.replace(spec, data_model=new_dm)


def _revise_field_rename(spec: AppSpecLike, stage: str, old_name: str, new_name: str) -> AppSpecLike:
...
def _revise_list_value(spec: AppSpecLike, list_name: str, old_value: str, new_value: str) -> AppSpecLike:
    ...
    new_md = dataclasses.replace(spec.master_data, lists=_seq_replace(spec.master_data.lists, new_lists))
    result = dataclasses.replace(spec, master_data=new_md)

    backed_field_names = {
        f.name for f in _seq(result.data_model.fields)
    ...
    for rp in _seq(result.routing):
        ...
        new_routing.append(dataclasses.replace(
            rp, options=new_options, route_per_option=_mapping_like(rp.route_per_option, mapping),
        ))
    result = dataclasses.replace(result, routing=_seq_replace(result.routing, new_routing))
        ...
        for fill in _seq(case.fills):
            ...
            for fname in backed_field_names:
                if fv.get(fname) == old_value:
                    ...
                    changed = True
            new_fills.append(
                dataclasses.replace(fill, values=_mapping_like(fill.values, fv)) if changed else fill
            )
...
def _revise_table_max_rows(spec: AppSpecLike, table_name: str, value: str) -> AppSpecLike:
    ...
    new_dm = dataclasses.replace(spec.data_model, tables=_seq_replace(spec.data_model.tables, new_tables))
    return dataclasses.replace(spec, data_model=new_dm)
