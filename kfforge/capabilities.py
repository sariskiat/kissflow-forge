...
def _matches(meta: dict[str, Any], body: str, query: str) -> bool:
    q = query.lower()
    haystack = " ".join(
        str(meta.get(k, "")) for k in ("id", "name", "ui_path", "status")
    ) + " " + body
...
def search_capabilities(query: str = "") -> dict[str, Any]:
    ...
    if not query:
        ...
        for path in docs:
            try:
                ...
                continue
            index.append({"id": meta.get("id"), "name": meta.get("name"),
                          "status": meta.get("status"), "modules": meta.get("modules")})
        return {"query": query, "count": len(index), "index": index, "errors": errors,
               "isError": False}
    ...
    for path in docs:
        ...
        entries.append({
            "id": meta.get("id"), "name": meta.get("name"), "status": meta.get("status"),
            "modules": meta.get("modules"), "ui_path": meta.get("ui_path"),
            "params": meta.get("params"), "body": body.strip(), "shapes": shapes,
        })
    return {"query": query, "count": len(entries), "entries": entries, "errors": errors,
           "isError": False}
