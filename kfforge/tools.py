...
def coerce_field_specs(param: str, value: Any) -> list[FieldSpec]:
    ...
    for i, d in enumerate(value):
        ...
        if not isinstance(d.get("name"), str) or not d["name"].strip():
            raise _shape_error(f"{at}['name']", d.get("name"), "a non-empty field name",
                               _FIELD_EXAMPLE)
        _field_type(f"{at}['type']", d.get("type"))
        opts = d.get("options")
        if opts is not None and not isinstance(opts, dict):
            raise _shape_error(f"{at}['options']", opts,
...
def coerce_layout(param: str, value: Any) -> dict[str, list[list[tuple[str, int, int]]]]:
    ...
    for section, rows in value.items():
        if not isinstance(rows, list):
            raise _shape_error(f"{param}[{section!r}]", rows,
                               "a list of rows, top to bottom", example)
        out_rows: list[list[tuple[str, int, int]]] = []
        for ri, row in enumerate(rows):
            if not isinstance(row, list):
                raise _shape_error(f"{param}[{section!r}][{ri}]", row,
                                   "a list of [field_name, Start, End] triples", example)
...
def coerce_table_columns(param: str, value: Any) -> list[tuple[str, str, dict[str, Any] | None]]:
    ...
    if not isinstance(value, list):
        raise _shape_error(param, value, "a list of [name, type] (or [name, type, options]) "
                                         "column entries", example)
    ...
    for i, col in enumerate(value):
        ...
        if opts is not None and not isinstance(opts, dict):
            raise _shape_error(f"{at}[2]", opts, "an object of opt-in per-column Field keys "
                                                 "written verbatim onto the node", example)
...
def coerce_validation_rules(param: str, value: Any) -> dict[str, list[tuple[str, str]]]:
    ...
    if not isinstance(value, dict):
        raise _shape_error(param, value, "an object of field name -> list of [operator, value] "
                                         "rules", example)
    norm: dict[str, list[tuple[str, str]]] = {}
    for field, rules in value.items():
        if not isinstance(rules, list):
            raise _shape_error(f"{param}[{field!r}]", rules,
                               "a list of [operator, value] rules", example)
...
def _coerce_step_pairs(param: str, value: Any) -> list[tuple[str, str | None]]:
    if not isinstance(value, list):
        raise _shape_error(param, value, "a list of [step name, role id or null] pairs",
                           _STEPS_EXAMPLE)
    ...
    for i, step in enumerate(value):
        ...
        if role is not None and not isinstance(role, str):
            raise _shape_error(f"{at}[1]", role, "an AppRole id, or null for an unassigned step",
                               _STEPS_EXAMPLE)
...
def coerce_parallel(
    ...
    example = ('{"name": "Route", "branches": [["Standard", [["Approve", null]]], '
    ...
    if not isinstance(branches, list):
        raise _shape_error(f"{param}['branches']", branches,
                           "a list of [branch name, steps] pairs", example)
...
def coerce_events(param: str, value: Any) -> dict[str, list[tuple[str | None, str]]]:
    ...
    if not isinstance(value, dict):
        raise _shape_error(param, value, "an object of field name -> list of [trigger, script] "
                                         "pairs", example)
    conv: dict[str, list[tuple[str | None, str]]] = {}
    for field, specs in value.items():
        if not isinstance(specs, list):
            raise _shape_error(f"{param}[{field!r}]", specs,
                               "a list of [trigger, script] pairs", example)
        pairs: list[tuple[str | None, str]] = []
        for i, spec in enumerate(specs):
            at = f"{param}[{field!r}][{i}]"
            if not isinstance(spec, (list, tuple)) or len(spec) != 2:
                raise _shape_error(at, spec, "a [trigger, script] pair — pass null for the "
                                             "trigger to derive it from the field's live type",
                                   example)
            trigger, script = spec
            if trigger is not None and not isinstance(trigger, str):
                raise _shape_error(f"{at}[0]", trigger,
                                   "a trigger string, or null to derive it", example)
...
def coerce_page_steps(param: str, value: Any) -> list[tuple[str, dict[str, Any]]]:
    ...
    example = ('[{"kind": "widget", "kwargs": {"container_id": "Container001", '
    ...
    for i, step in enumerate(value):
        ...
        if not isinstance(kind, str) or not kind.strip():
            raise _shape_error(f"{at}['kind']", kind, "one of container|widget|popup|event|"
                                                      "style|bind|design", example)
        kwargs = step.get("kwargs", {})
        if kwargs is None:
            kwargs = {}
        if not isinstance(kwargs, dict):
            raise _shape_error(f"{at}['kwargs']", kwargs,
                               "an object of arguments for that builder", example)
...
def coerce_case_steps(param: str, value: Any) -> list[dict[str, Any]]:
    ...
    example = ('[{"name": "Manager Approve", "values": {"Urgency": "High"}, "reject": false, '
               '"comment": ""}]')
    if not isinstance(value, list):
    ...
    for i, step in enumerate(value):
        ...
        if not isinstance(values, dict):
            raise _shape_error(f"{at}['values']", values,
                               "an object of field name (or id) -> value", example)
        comment = step.get("comment", "")
        if not isinstance(comment, str):
            raise _shape_error(f"{at}['comment']", comment, "a comment string", example)
        plans.append({"name": name, "values": values,
                      "reject": bool(step.get("reject", False)), "comment": comment})
    return plans
