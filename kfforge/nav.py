...
(returns a NEW draft, never mutates its input) and fails loud (ValueError), matching kfforge.graph
and kfforge.pages. `_instantiate`/`_keep_subset`/`load_shape` are reused from kfforge.pages rather
...
from .pages import _instantiate, _keep_subset, load_shape
...
def list_navigation(app_draft: Draft) -> dict[str, list[str]]:
    ...
    return {nid: list(n.get("Navigation::Menu") or []) for nid, n in _kind(app_draft, "Navigation").items()}
...
def add_page_menu(app_draft: Draft, *, nav_id: str, page_id: str, label: str) -> Draft:
    ...
    subset = _keep_subset(shape["template"], ("Menu_Sample01", "FieldMapping_Sample01", "Property_Sample01"))
    nodes, idmap = _instantiate(subset, external={"Navigation_Sample01": nav_id, "Page_Sample01": page_id})
