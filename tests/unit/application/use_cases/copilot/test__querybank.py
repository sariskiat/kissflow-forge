...
Ported verbatim from `tests/test_querybank.py` (pre-refactor), now importing
`app.application.use_cases.copilot._querybank` (refactor spec G11, Appendix
B: every pure `application/` module moves into its owning family's
`use_cases/` folder).
"""
...
from app.application.use_cases.copilot._querybank import (
    ANSWER,
    GRAPH_DIFF,
    MODULES,
    SWEEPS,
