"""Spec for app.application.models.requests.design.forge_render_mockups_request."""
...
def test_constructs_from_a_plain_spec_dict() -> None:
    req = ForgeRenderMockupsRequest(spec=blank_spec().model_dump(mode="json"))
    assert req.spec.app_name == ""
    assert req.out_dir is None


def test_carries_out_dir() -> None:
    req = ForgeRenderMockupsRequest(
