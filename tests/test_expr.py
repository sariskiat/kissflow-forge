"""Spec for app.domain.expr — Expression/Node AST builder for branch conditions and GotoTask loop
...
def test_build_branch_condition_accepts_text_field(
    ...
    DATA_TYPE comment in app.domain.expr for why it, and only it besides Select, is included."""
    got = build_branch_condition(
        clean_draft,
        process_def_id=branch_pd_id,
...
def test_build_branch_condition_unknown_field_rejected(
    clean_draft: Draft, branch_pd_id: str
) -> None:
    before = copy.deepcopy(clean_draft)
    with pytest.raises(ValueError):
        build_branch_condition(
...
def test_build_goto_gate_unknown_activity_rejected(
    clean_draft: Draft, boolean_field_id: str
) -> None:
    before = copy.deepcopy(clean_draft)
    with pytest.raises(ValueError):
        build_goto_gate(
            clean_draft, goto_activity_id="Activity_DoesNotExist99", field_id=boolean_field_id
        )
    assert clean_draft == before
...
def test_rewire_unknown_expression_leaves_draft_untouched(
    ...
    with pytest.raises(ValueError):
        rewire_condition(
            draft, expression_id="Expression_DoesNotExist99", new_field_id=urgency_field_id
        )
    assert draft == before


def test_rewire_rejects_property_owned_expression(
    clean_draft: Draft, urgency_field_id: str
) -> None:
    ...
    with pytest.raises(ValueError):
        rewire_condition(
            draft, expression_id="Expression_SamplePrefix01", new_field_id=urgency_field_id
        )
    assert draft == before
...
def test_rewire_goto_condition_rejects_new_literal(
    ...
    with pytest.raises(ValueError):
        rewire_condition(
            draft, expression_id=expr_id, new_field_id=deep_done_field_id, new_literal="true"
        )
    assert draft == before
