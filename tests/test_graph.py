"""Wave-1 unit spec for the offline graph engine. RED until app.domain.graph exists.
...
from app.domain.graph import apply_changes, parse_draft
...
def test_user_field_gets_a_querydefinition_sibling():
    ...
    fid = next(k for k, v in new.items() if isinstance(v, dict) and v.get("Type") == "User")
    field = new[fid]
...
def test_user_field_lhsmodel_override_lands_on_the_querydefinition():
    new = apply_changes(
        _load(),
        [FieldSpec(name="Employee", type=FieldType.USER, options={"LHSModel": "_employee"})],
    )
    fid = next(k for k, v in new.items() if isinstance(v, dict) and v.get("Type") == "User")
...
def test_bare_process_gets_a_processdef_scaffold() -> None:
    ...
    from app.domain.graph import ensure_process_def
...
def test_fields_can_be_added_on_top_of_a_scaffolded_process() -> None:
    from app.domain.graph import apply_changes, ensure_process_def, field_names
    from app.domain.types import FieldSpec, FieldType

    bare = {"Root": "M1", "M1": {"Id": "M1", "Kind": "Model", "Name": "P", "FlowType": "Process"}}
    got = apply_changes(
        ensure_process_def(bare, ("Submit",)), [FieldSpec(name="Title", type=FieldType.TEXT)]
    )
...
def test_columns_tile_the_row_grid_and_never_overflow() -> None:
    ...
    """
    from app.domain.graph import FIELD_SPAN, ROW_UNITS, apply_changes
    from app.domain.types import FieldSpec, FieldType
...
def test_per_type_defaults_match_what_the_builder_writes() -> None:
    ...
    by_name = {
        n["Name"]: n for n in got.values() if isinstance(n, dict) and n.get("Kind") == "Field"
    }
...
def test_regroup_puts_each_field_in_its_named_section() -> None:
    ...
    got = regroup_into_sections(
        _draft_with_fields("a", "b", "c", "d"), [("Step 1", ["a", "b"]), ("Step 2", ["c", "d"])]
    )
...
def test_regroup_creates_an_empty_banner_section_in_place() -> None:
    ...
    got = regroup_into_sections(
        _draft_with_fields("a", "b"), [("Before", ["a"]), ("Banner", []), ("After", ["b"])]
    )
...
def test_regroup_never_drops_an_unlisted_field() -> None:
    from app.domain.graph import field_names, regroup_into_sections
...
def test_regroup_still_respects_the_row_grid() -> None:
    from app.domain.graph import FIELD_SPAN, ROW_UNITS, regroup_into_sections
...
def test_apply_exact_layout_places_fields_at_stated_coordinates() -> None:
    ...
    from app.domain.graph import apply_exact_layout, regroup_into_sections

    base = regroup_into_sections(
        _draft_with_fields("a", "b", "c", "d"), [("S", ["a", "b", "c", "d"])]
    )
    layout = {
        "S": [
    ...
    assert len(rows) == 3
    coords = {col_name[c]: (got[c]["Start"], got[c]["End"]) for r in rows for c in r["Row::Column"]}
...
def _section_rows(draft: dict, section_name: str = "S") -> list[list[tuple[str, int, int]]]:
    ...
    sec = next(
        ...
        if isinstance(v, dict) and v.get("Type") == "Section" and v.get("Name") == section_name
    )
    col_name = {
        f["Column"]: f["Name"]
    ...
    return [
        [(col_name[c], draft[c]["Start"], draft[c]["End"]) for c in draft[rid]["Row::Column"]]
...
def test_apply_exact_layout_leftovers_never_run_off_the_grid() -> None:
    ...
    """
    from app.domain.graph import ROW_UNITS, apply_exact_layout, regroup_into_sections
...
def test_apply_exact_layout_leftover_order_survives_a_different_hash_seed() -> None:
    ...
    src = (  # noqa: UP031
        ...
        "from app.domain.graph import apply_changes, apply_exact_layout, regroup_into_sections;"
        "from app.domain.types import FieldSpec, FieldType;"
        "names=['Ticket No','Contact Date','Problem','Unit Serial','Urgency','Outcome'];"
        "d={'Root':'M1','M1':{'Id':'M1','Kind':'Model','Name':'F','FlowType':'Form'}};"
    ...
    for seed in ("1", "424242"):
        ...
        outs.append(
            subprocess.run(
                [sys.executable, "-c", src], check=True, env=env, capture_output=True, text=True
            ).stdout.strip()
        )
...
def _prod_capture_widest_row() -> list[tuple[int, int]]:
    ...
    shape = _json.loads(
        (
            ...
            / "shapes"
            / "process_template_identity_shell.json"
...
def test_layout_guard_accepts_the_four_column_row_the_prod_capture_proves() -> None:
    ...
    from app.domain.graph import apply_exact_layout, regroup_into_sections, validate_layout_spans
...
def test_layout_guard_still_refuses_what_the_capture_does_back() -> None:
    ...
    from app.domain.graph import validate_layout_spans
...
def test_apply_exact_layout_sets_section_descriptions() -> None:
    ...
    got = apply_exact_layout(base, {"S": [[("a", 0, 6)]]}, descriptions={"S": rich})
    sec = next(v for v in got.values() if isinstance(v, dict) and v.get("Type") == "Section")
...
def test_build_workflow_sweeps_dangling_permission_backrefs() -> None:
    ...
    """
    from app.domain.graph import build_workflow
...
def test_add_goto_task_wires_backward_jump_and_backref() -> None:
    from app.domain.graph import add_goto_task
...
def test_add_goto_task_branch_process_def_id_lands_inside_that_branch_not_root() -> None:
    ...
    a1_id = next(
        a for a in draft[branch_a_pd_id]["ProcessDef::Activity"] if draft[a]["Name"] == "A1"
    )

    got, goto_id = add_goto_task(
        draft, target_activity_id=a1_id, branch_process_def_id=branch_a_pd_id
    )
...
def test_add_goto_task_branch_process_def_id_matches_default_when_target_already_in_branch(
    ...
    from app.domain import graph as graph_mod
    from app.domain.graph import add_goto_task
    ...
    with_branch, goto_id_1 = add_goto_task(
        draft, target_activity_id=a1_id, branch_process_def_id=branch_a_pd_id
    )
...
def test_add_goto_task_branch_process_def_id_rejects_target_outside_that_branch() -> None:
    ...
    draft, root_pd_id, branch_a_pd_id, _branch_b_pd_id = _process_with_parallel_branches()
    root_step_2_id = next(
        a for a in draft[root_pd_id]["ProcessDef::Activity"] if draft[a]["Name"] == "Root Step 2"
    )
    before = copy.deepcopy(draft)

    with pytest.raises(ValueError, match="never cross-branch"):
        add_goto_task(
            draft, target_activity_id=root_step_2_id, branch_process_def_id=branch_a_pd_id
        )
    assert draft == before
...
def test_add_goto_task_branch_process_def_id_rejects_target_in_sibling_branch() -> None:
    ...
    b1_id = next(
        a for a in draft[branch_b_pd_id]["ProcessDef::Activity"] if draft[a]["Name"] == "B1"
    )
    before = copy.deepcopy(draft)
...
def test_add_goto_task_branch_process_def_id_rejects_unknown_process_def() -> None:
    ...
    with pytest.raises(ValueError):
        add_goto_task(
            draft, target_activity_id=a1_id, branch_process_def_id="ProcessDef_DoesNotExist99"
        )
    assert draft == before
...
def test_add_goto_task_root_chain_behavior_unchanged_when_a_parallel_also_exists() -> None:
    ...
    root_step_1_id = next(
        a for a in draft[root_pd_id]["ProcessDef::Activity"] if draft[a]["Name"] == "Root Step 1"
    )
...
def test_build_workflow_two_sequential_parallels() -> None:
    ...
    parallel_nodes = [
        n
        for n in got.values()
        if isinstance(n, dict) and n.get("Kind") == "Activity" and n.get("NodeType") == "Parallel"
    ]
...
def test_build_workflow_single_parallel_unchanged() -> None:
    ...
    assert [draft[a]["Name"] for a in draft[branch_a_pd_id]["ProcessDef::Activity"]] == ["A1", "A2"]
    assert [draft[a]["Name"] for a in draft[branch_b_pd_id]["ProcessDef::Activity"]] == ["B1", "B2"]
...
def test_add_goto_task_can_pair_with_build_goto_gate_and_reads_clean() -> None:
    ...
    from app.domain.expr import build_goto_gate
...
def test_field_override_matrix_branch_aware_per_step_rule() -> None:
    ...
    from app.domain.graph import apply_changes, field_override_matrix
...
def test_add_table_per_column_options_override_type_defaults() -> None:
    ...
    cols = {n["Name"]: n for n in got.values() if isinstance(n, dict) and n.get("Kind") == "Field"}
...
def test_add_table_writes_a_select_child_that_names_its_list() -> None:
    ...
    bare = {"Root": "M1", "M1": {"Id": "M1", "Kind": "Model", "Name": "F", "FlowType": "Form"}}
    got = add_table(
        bare, "Items", [("Item", "Text", None), ("Grade", "Select", {"ReferredList": "List_G1"})]
    )
    (grade,) = [
        n
        for n in got.values()
...
def test_add_sequence_number_builds_field_props_expression_and_hidden_column() -> None:
    ...
    seqs = [n for n in again.values() if isinstance(n, dict) and n.get("Type") == "SequenceNumber"]
    assert len(seqs) == 1
...
def test_add_field_validation_builds_criteria_and_condition() -> None:
    ...
    from app.domain.graph import add_field_validation, apply_changes
    from app.domain.types import FieldSpec, FieldType
...
def test_add_field_validation_writes_error_message_when_given() -> None:
    ...
    got = add_field_validation(
        d, "Notes", "MAX_LENGTH", "10", error_message="Maximum length is 10 characters"
    )
    fld = next(
        n
        for n in got.values()
...
def test_set_field_computed_builds_function_ast_with_field_and_static_args() -> None:
    ...
    from app.domain.graph import apply_changes, set_field_computed
    from app.domain.types import FieldSpec, FieldType
...
def test_set_field_computed_raises_on_missing_field_refs() -> None:
    ...
    with pytest.raises(ValueError, match="no field named"):
        set_field_computed(d, "Computed Sample", {"fn": "concatenate", "args": [{"field": "Nope"}]})
...
def test_set_conditional_visibility_builds_columnvisibility_criteria() -> None:
    ...
    from app.domain.graph import apply_changes, set_conditional_visibility
    from app.domain.types import FieldSpec, FieldType
...
def test_add_table_after_section_places_host_adjacent_to_banner() -> None:
    ...
    from app.domain.graph import add_table, apply_changes, regroup_into_sections
    from app.domain.types import FieldSpec, FieldType

    bare = {"Root": "M1", "M1": {"Id": "M1", "Kind": "Model", "Name": "F", "FlowType": "Form"}}
    draft = apply_changes(
        bare, [FieldSpec(name="A", type=FieldType.TEXT), FieldSpec(name="B", type=FieldType.TEXT)]
    )
...
def _three_section_form() -> dict:
    from app.domain.graph import apply_changes, regroup_into_sections
    from app.domain.types import FieldSpec, FieldType
...
def test_set_section_style_full_form_chain_count_is_n_plus_one() -> None:
    ...
    from app.domain.graph import set_section_style
...
def test_set_section_style_accepts_explicit_ref_and_value_dicts() -> None:
    ...
    got = set_section_style(
        _three_section_form(), {"S1": {"Section.Bg.Color": {"value": "#112233"}}}
    )
    sec = next(
        v
        for v in got.values()
...
def test_regroup_does_not_break_a_table_bearing_flow() -> None:
    ...
    from app.domain.graph import add_table, regroup_into_sections
    from app.domain.types import FieldType

    draft = regroup_into_sections(
        _draft_with_fields("A", "B"), [("Head", ["A"]), ("Log Banner", []), ("Tail", ["B"])]
    )
    ...
    from app.domain.graph import apply_changes, merge_groups
    from app.domain.types import FieldSpec
...
def test_delete_nodes_sweeps_a_user_fields_query_definition() -> None:
    ...
    from app.domain.graph import delete_closure, delete_nodes

    draft = _form_with(FieldSpec(name="Owner", type=FieldType.USER))
    qids = [
        k for k, v in draft.items() if isinstance(v, dict) and v.get("Kind") == "QueryDefinition"
    ]
...
def test_delete_nodes_sweeps_the_fields_own_computed_expression_tree() -> None:
    from app.domain.graph import delete_nodes, set_field_computed

    draft = _form_with(
        FieldSpec(name="Total", type=FieldType.NUMBER), FieldSpec(name="Qty", type=FieldType.NUMBER)
    )
    draft = set_field_computed(
        draft, "Total", {"fn": "concatenate", "args": [{"static": "n="}, {"field": "Qty"}]}
    )
    ...
    assert _scalar_dangling_refs(got) == []
    assert "Qty" in {
        v.get("Name") for v in got.values() if isinstance(v, dict) and v.get("Kind") == "Field"
    }


def test_delete_nodes_sweeps_the_fields_own_validation_criteria_and_conditions() -> None:
    from app.domain.graph import add_field_validation, delete_nodes
...
def test_delete_nodes_sweeps_the_fields_own_conditional_visibility_rule() -> None:
    ...
    from app.domain.graph import delete_nodes, set_conditional_visibility
...
def test_delete_nodes_still_sweeps_the_column_and_its_permissions() -> None:
    ...
    from app.domain.graph import delete_nodes
...
def test_field_delete_blockers_is_empty_for_a_plain_field() -> None:
    from app.domain.graph import field_delete_blockers
...
def test_field_delete_blockers_names_a_surviving_formula_that_reads_the_field() -> None:
    ...
    from app.domain.graph import field_delete_blockers, set_field_computed
...
def test_field_delete_blockers_names_a_conditional_visibility_trigger() -> None:
    from app.domain.graph import field_delete_blockers, set_conditional_visibility
...
def test_field_delete_blockers_names_a_surviving_event_script_that_uses_the_id() -> None:
    ...
    from app.domain.graph import field_delete_blockers, set_field_events
...
def test_delete_closure_sweeps_table_and_its_cluster() -> None:
    from app.domain.graph import add_table, delete_closure, delete_nodes
...
def test_delete_closure_unknown_table_raises() -> None:
    from app.domain.graph import delete_closure
...
def test_delete_closure_table_and_node_tree_edge_cases() -> None:
    from app.domain.graph import _node_tree, delete_closure
...
def test_apply_changes_writes_a_wired_select() -> None:
    ...
    got = apply_changes(
        _load(), [FieldSpec(name="Urgency", type=FieldType.SELECT, referred_list="List_Sample01")]
    )
...
def test_repack_layout_pure_does_not_mutate_input() -> None:
    ...
    from app.domain.graph import repack_layout
...
def test_repack_layout_default_widths_and_column_stretching() -> None:
    ...
    intake_sec = next(
        ...
        if isinstance(v, dict) and v.get("Type") == "Section" and v.get("Name") == "Intake"
    )
...
def test_repack_layout_custom_widths_and_capping() -> None:
    ...
    wrap_sec = next(
        v
        for v in repacked.values()
        if isinstance(v, dict) and v.get("Type") == "Section" and v.get("Name") == "Wrap-up"
    )
...
def test_repack_layout_section_and_step_descriptions() -> None:
    ...
    step_node = next(
        v
        for v in repacked.values()
        if isinstance(v, dict) and v.get("Kind") == "Activity" and v.get("Name") == "Ticket arrives"
    )
    ...
    other_sec = next(
        v
        for v in repacked.values()
        if isinstance(v, dict) and v.get("Type") == "Section" and v.get("Name") == "Other"
    )
...
def test_repack_layout_introduces_no_new_doctor_problems() -> None:
    """repack_layout rewrites row/column geometry; nothing it does may make the graph less
    healthy than it already was.

    This assertion used to read `assert report.ok, report.violations` — `ok` is a METHOD, so the
    bare attribute was a bound-method object and always truthy. The test therefore passed without
    ever running the audit, and `violations` (not a field on DoctorReport; the field is
    `problems`) was never evaluated either, because an assert only builds its message on failure.

    Calling it revealed the real state: the synthetic fixture carries 13 pre-existing problems,
    all of the form "section X is never editable at any live step" — a property of the fixture's
    permission matrix, not of layout. So absolute cleanliness was never the right claim. The
    DIFFERENTIAL is: repack_layout must add nothing. That is what this function is responsible
    for, and unlike the original it actually executes.
    """
    from synthetic import synthetic_process_draft
    ...
    draft = synthetic_process_draft()
    before = doctor(draft)
    after = doctor(repack_layout(draft))
    ...
    assert len(after.problems) == len(before.problems), (before.problems, after.problems)
