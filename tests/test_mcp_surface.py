...
from app.infrastructure.kissflow.client import ApplyReport
...
def test_the_citation_check_actually_sees_citations() -> None:
    ...
    cited = {c for t in _emitted().values() for c in _TOOL_CITATION.findall(t.description or "")}
...
def test_a_parameter_whose_description_enumerates_its_values_is_never_a_bare_string(
    ...
    t = _emitted()[tool_name]
    findings = enum_lint(t.description or "", (t.input_schema or {}).get("properties") or {})
    assert not findings, (
...
def test_the_enum_lint_fires_on_every_vocabulary_the_boundary_currently_types() -> None:
    ...
    hits = {
        ...
        for param, _values in enum_lint(
            t.description or "",
            {p: {"type": "string"} for p in ((t.input_schema or {}).get("properties") or {})},
        )
    }
...
def test_the_enum_lint_flags_an_open_parameter_but_not_a_closed_one() -> None:
    ...
    assert enum_lint(desc, {"mode": {"type": "string"}}) == [("mode", ["fast", "slow", "careful"])]
    assert enum_lint(desc, {"mode": {"type": "string", "enum": ["fast", "slow", "careful"]}}) == []
    assert (
        enum_lint(
            desc,
...
# per tool (59 of them, each mirroring a Report dataclass that already exists in app.infrastructure.kissflow.client /
# app.infrastructure.kissflow.pages_live), which is real follow-on work and not a boundary-annotation change.
#
...
def test_a_payload_level_failure_is_also_a_protocol_level_failure(
    tool_name: str, args: dict[str, Any], no_kf_env: None
) -> None:
...
def test_the_promotion_is_a_middleware_not_a_per_tool_habit() -> None:
    ...
    def fails() -> dict[str, Any]:
        """A tool that reports failure the way every tool in app.infrastructure.mcp.server does."""
...
def test_an_off_grid_layout_span_leaves_the_boundary_as_data_not_a_traceback(
    ...
    from app.infrastructure.kissflow.client import apply_fields_and_layout
...
def test_a_silently_ignored_type_change_is_an_error_on_the_protocol_envelope(
    ...
    second = srv.kf_apply_field_change(**args)  # ty: ignore[invalid-argument-type]
...
def test_a_report_carrying_only_collateral_damage_is_not_reported_as_an_error() -> None:
    ...
    warned = ApplyReport(
        ...
        missing=(),
        changed_ignored=(),
        collateral=("'b' was re-tiled",),
        remediation=("forge_apply_layout",),
        meta_version="v2",
        published=False,
    )
    ...
    failed = ApplyReport(
        ...
        collateral=(),
        remediation=("forge_delete_fields",),
        meta_version="v2",
        published=False,
    )
...
def test_forge_list_app_roles_returns_only_the_roles_scoped_to_the_app(
    ...
    assert got["roles"] == [{"_id": "Ro1", "Name": "Requester"}, {"_id": "Ro2", "Name": "Approver"}]
...
def test_every_client_is_told_the_group_rule_in_the_handshake() -> None:
    ...
    async def _handshake() -> str:
        async with Client(mcp) as client:
            # fastmcp 4's default connect mode is "auto": a modern-era handshake leaves
            # `initialize_result` None, and `client.instructions` is the era-neutral accessor
            # upstream tells you to read instead. Same channel, still the initialize handshake.
