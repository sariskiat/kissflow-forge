"""app.application.intake.serde -- exact dict round-trip for AppSpec: the wire format a stateless MCP
tool passes in and returns on every call, since the server itself holds no session state
(src/app/server.py's forge_* intake/design tools take a spec dict as an argument and hand a spec
...
(`typing.get_type_hints`, resolved against `app.application.intake.schema`'s module globals, since every
...
`options`, `config`, `values` on `StepFill`, ...) is `app.application.intake.schema`'s OWN on-wire shape
...
because `app.infrastructure.mcp.server` needs the exact same dataclass/enum/tuple -> JSON-safe conversion for
OTHER dataclasses this package hands back from a tool call (`app.application.intake.compile.BuildPlan`/
`Op`, `app.application.design.ConfirmationRequest`, `app.application.intake.questions.Question`) without ever
...
actually answered (the same silent-drop failure `app.application.intake.schema`'s own docstring warns
...
if TYPE_CHECKING:
    # typeshed-only protocol for \"is a dataclass type\"; never imported at runtime.
...
def to_wire(value: Any) -> Any:
    ...
    uses string keys -- `app.application.intake.compile.Op.args` -- this is defensive, not load-bearing).
...
@functools.lru_cache
def _field_types(cls: type[DataclassInstance]) -> Mapping[str, Any]:
    """Field name -> resolved (real, not string) type annotation for a dataclass, via
    `typing.get_type_hints`. Needed because `app.application.intake.schema` (like this module) writes
...
def _has_default(cls: type[DataclassInstance], name: str) -> bool:
    ...
    raise KeyError(
        name
    )  # pragma: no cover -- only ever called with a name _field_types just gave us
...
def _from_wire(value: Any, typ: Any, *, path: str) -> Any:
    ...
    if origin is tuple:
        if not isinstance(value, list):
            raise ValueError(
                f"{path}: expected a list for a tuple field, got {type(value).__name__}"
            )
        args = typing.get_args(typ)
    ...
    if typ in (str, int, float):
        if not isinstance(value, typ) or isinstance(value, bool):
            raise ValueError(
                f"{path}: expected {typ.__name__}, got {type(value).__name__}: {value!r}"
            )
        return value
...
def spec_from_dict(data: dict[str, Any]) -> AppSpec:
    ...
    if not isinstance(data, dict):
        ...
        # app.application.intake.compile's own precedent, same rule suppressed the same way there); a
