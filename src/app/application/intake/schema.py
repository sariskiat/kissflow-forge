"""app.application.intake.schema — the 11-dimension input spec a Kissflow build needs before a single
...
`FieldReq.type`/`TableColumnReq.type` are `app.domain.types.FieldType`, `VisibilityEntry.permission`
is `app.domain.types.Visibility`, and `ComputedReq.trigger` is `app.domain.types.EventTrigger` (born
...
branches. `app.domain.graph.build_workflow` supports a `parallel`/`parallel_after` gateway
...
# `app.domain.types` — the neutral module both this spec layer and the live write path
...
# existing `from app.application.intake.schema import EventTrigger` still resolves to the one enum.
...
@dataclass(frozen=True)
class WidgetIntent:
    """One widget on a page, identified by its `Script.web` slug (`app.domain.pages.WIDGET_SLUGS` —
    ...
    (key, value) string pairs; `app.domain.pages.WIDGET_REQUIRED_CONFIG` names, per slug, which keys
...
def blank_spec() -> AppSpec:
    ...
    `app_name` is `""`, `approved` is `False`). Added for `app.infrastructure.mcp.server`'s stateless
...
def _gap_dimensions(spec: AppSpec) -> dict[int, str]:
    ...
    if not (pg.pain and pg.goal and pg.done_definition and pg.terminal_states and pg.result_values):
        flag(
            1,
            "need the pain, the goal, a done-definition, at least one terminal state, and "
    ...
    if not (spec.master_data.lists or spec.master_data.confirmed_none):
        flag(
            7,
            "need at least one reference list, OR an explicit confirmation there is none "
