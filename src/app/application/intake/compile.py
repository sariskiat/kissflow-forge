"""app.application.intake.compile — turns an APPROVED, COMPLETE AppSpec into a BuildPlan: the ordered
...
from app.application.intake.serde import to_wire
from app.domain import coverage
from app.domain.pages import WIDGET_REQUIRED_CONFIG, WIDGET_SLUGS
...
# CLAUDE.md and `app.domain.graph.build_workflow`'s own separate `Resource`-node write).
OP_ORDER: tuple[str, ...] = (
    "create_process",
    "member_batch",
    "create_list",
...
def _check_no_duplicate_branch_names(spec: AppSpec) -> None:
    ...
    belongs to) ambiguous. Mirrors `app.domain.graph.build_workflow`'s own runtime guard, but refuses
...
def _check_widgets(spec: AppSpec) -> None:
    """A widget slug outside `app.domain.pages.WIDGET_SLUGS`, or missing a config key
    `app.domain.pages.WIDGET_REQUIRED_CONFIG` demands for that slug (`add_widget` itself refuses to
            ...
            for w, where in _page_widgets(page):
                if w.slug not in WIDGET_SLUGS:
                    raise ValueError(
                        ...
                        f"(app.domain.pages.WIDGET_SLUGS): {sorted(WIDGET_SLUGS)}"
                    )
                ...
                for key in required:
                    ...
                    if not config.get(key):
                        raise ValueError(
                            ...
                            f"{key!r} (app.domain.pages.WIDGET_REQUIRED_CONFIG)"
                        )
    ...
    def walk(node: DesignNode, where: str) -> None:
        ...
        if node.kind == "container":
            ...
            if node.widget is None:
                raise ValueError(f"{where}: a design widget ({node.name!r}) has no WidgetIntent")
    ...
    for case in spec.test_cases.cases:
        ...
        for fill in case.fills:
            ...
            for field_name, value in fill.values:
                ...
                if field is None:
                    raise ValueError(f"test case {case.name!r} fills unknown field {field_name!r}")
...
def _op_create_list(spec: AppSpec) -> tuple[Op, ...]:
    ...
    return tuple(
        Op(
            ...
            why=(
                "HUMAN-GATED (personal_data, PDPA): this list holds personal data — a human "
                ...
                else "execute with forge_create_list (create-or-reuse by name, REPLACE-semantics "
...
def _workflow_decomposition(spec: AppSpec) -> tuple[tuple[str, ...], tuple[dict[str, Any], ...]]:
    ...
    DECLARATIVE plan a later apply-executor maps onto `app.domain.graph.build_workflow`: a linear
    ...
    `app.domain.graph.build_workflow`, which already supports N sequential gateways). (No executor
    consumes it yet — `app.application.engine` is still a planning stub; the plan is the deliverable a
    build agent reads.)
...
def _op_build_workflow(spec: AppSpec) -> tuple[Op, ...]:
    ...
    matching `app.domain.graph.build_workflow`'s own `Resource`-node write being a materially
    ...
    into the executable `steps` + `parallels` pair `app.domain.graph.build_workflow` actually takes —
...
def _op_set_assignees(spec: AppSpec) -> tuple[Op, ...]:
    ...
    that stage's Activity, matching `app.domain.graph.build_workflow`'s own `roles`/`Step`
    ...
    for lst in spec.rework_loops.loops:
        ...
        ops.append(
            Op(
                ...
                why=f"backward GotoTask {lst.from_stage!r} -> {lst.to_stage!r}, Boolean-gated, fail-closed "
                f"(Gate polarity)"
                + (
...
def _op_set_events(spec: AppSpec) -> tuple[Op, ...]:
    ...
    for c in spec.data_model.computed:
        ...
        unverified = sorted(s for s in c.source_fields if type_of[s] not in TRIGGER_LIVE_CONFIRMED)
        ops.append(
            Op(
                kind="set_events",
                args={
                ...
                why=(
                    f"computed {c.target_field!r} via an event on its source field(s), never the "
