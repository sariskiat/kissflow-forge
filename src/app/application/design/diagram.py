"""app.application.design.diagram -- draw.io / mxGraph XML for the flow shape and the data shape.
...
app.application.design.AppSpecLike -- see the package docstring for why this never imports
app.application.intake) and never touches the network or the Kissflow builder API.

Shape adapters (`_seq`/`_seq_replace`) live in this module because it is the one every other
app.application.design submodule already imports from. app.application.intake.schema.AppSpec (the real spec)
...
if TYPE_CHECKING:
    from app.application.design import AppSpecLike, LoopLike
...
_ROW_STYLE = (
    "text;strokeColor=none;fillColor=none;align=left;verticalAlign=middle;spacingLeft=6;html=1;"
)
...
# app.application.intake's real wrapper-per-dimension shape.
# ---------------------------------------------------------------------------------------------


def _seq(x: Any) -> list[Any]:
    """Return the sequence `x` names, whether `x` IS a bare sequence or is a dimension-wrapper
    dataclass (app.application.intake.schema's `Stages`/`Routing`/`ReworkLoops`/`TestCases`, each holding
    ...
    # attribute names actually seen across app.application.intake.schema's dimension wrappers.
...
def _loop_direction(idx: dict[str, int], lp: LoopLike) -> str:
    ...
    app.application.intake.schema.LoopSpec's own contract enforced by compile.py), "forward" (to_stage
...
def _label(item: Any) -> str:
    ...
    app.application.intake.schema.WidgetIntent has no `.name` at all (it has `.slug`), so this generic
...
def _column_label(col: Any) -> str:
    """ "name : type" (+ "*" when required) for a richer column object (app.application.intake.schema.
...
class _Canvas:
    ...
    def resolve(self, name: str, *, x: float | None = None, y: float | None = None) -> str:
        ...
        self._fallback_n += 1
        return self.vertex(
            name=name, value=_xml_escape(name), x=fx, y=fy, w=BOX_W, h=BOX_H, style=_FALLBACK_STYLE
        )
...
def schema_diagram_xml(spec: AppSpecLike) -> str:
    ...
    for stage_name, rows in sections.items():
        ...
        sid = c.vertex(
            name=None, value=_xml_escape(stage_name), x=x, y=y, w=260, h=h, style=_SECTION_STYLE
        )
    ...
    for t in _seq(spec.data_model.tables):
        ...
        h = 30 + len(rows) * 26
        tid = c.vertex(
            name=None, value=_xml_escape(t.name), x=x, y=y, w=240, h=h, style=_TABLE_STYLE
        )
        for i, row in enumerate(rows):
            c.vertex(
                name=None,
