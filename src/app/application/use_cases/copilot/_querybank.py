...
CLI: ``python3 -m app.application.use_cases.copilot._querybank [--sweep S]
[--observable O] [--count]``

Ported verbatim from `app.application.querybank` (refactor spec G11,
Appendix B: every pure `application/` module moves into its owning family's
`use_cases/` folder). No tool calls this module today -- it is the standing
catalog a future copilot discovery sweep script consumes, relocated here
because "copilot query bank" has exactly one sensible family home.
"""
...
class FieldType:
    ...
    modules: frozenset[
        str
    ]  # palettes it appears in (dataform assumed = process; sweep verifies)
...
_NO_SEQ_NO_SMART = frozenset(
    {"process", "dataform"}
)  # board palette lacks these (captured)
...
FIELD_TYPES: tuple[FieldType, ...] = (
    ...
    FieldType(
        "Multi-select dropdown", "multi-select-dropdown", "basic", "field-a", _ALL
    ),
...
ASPECTS: tuple[tuple[str, str, str, str], ...] = (
    ...
    (
        ...
        "settings",
        'In {flow}, make "{field}" required only when the "Priority Flag" Yes/No field is Yes.',  # noqa: E501
    ...
    (
        ...
        "computed toggle + formula/expression subtree (or copilot refusal — capture it)",  # noqa: E501
    ),
    (
        "validation-rule",
        "validation",
        'In {flow}, add a validation rule to "{field}" so invalid values are rejected with a message.',  # noqa: E501
    ...
    (
        "event",
        "events",
        'In {flow}, add an event to "{field}" that updates another field when it changes.',  # noqa: E501
    ...
    for ft in FIELD_TYPES:
        for module in MODULES:
            ...
            out.append(
                Query(
                    ...
                    expect=f"new Field node (type {ft.name}) under the root Model, with its Row/Column placement and per-type keys",  # noqa: E501
                )
            )
        ...
        out.append(
            Query(
                ...
                capability=f"field.{ft.slug}",
                observable=ANSWER,
                prompt=f"When should I use a {ft.name} field, and when is it the wrong choice?",  # noqa: E501
        ...
        out.append(
            Query(
                ...
                capability=f"field.{ft.slug}",
                observable=ANSWER,
                prompt=f"Does a {ft.name} field behave differently in a process, a board, and a dataform? What changes?",  # noqa: E501
...
def _pair_diff_queries() -> list[Query]:
    ...
    return [
        Query(
            ...
            observable=ANSWER,
            prompt=f"What is the difference between a {a} field and a {b} field, and how do I pick?",  # noqa: E501
...
def _module_semantic_queries() -> list[Query]:
    items: tuple[tuple[str, str, str], ...] = (
        (
            "boards",
            "process-vs-board",
            "When should an app be a board instead of a process? Give the decision rule.",  # noqa: E501
        ),
        (
            "boards",
            "board-statuses",
            "How do board statuses and columns work, and how do items move between them?",  # noqa: E501
        ),
        (
            "boards",
            "board-workflow",
            "Can a board have an approval workflow like a process? What are the limits?",  # noqa: E501
        ),
        (
            "dataform",
            "process-vs-dataform",
            "When should data live in a dataform instead of process fields?",
        ),
        (
            "dataform",
            "dataform-page-connection",
            "How do I connect a dataform to a process using a page, so process items can read and write dataform records?",  # noqa: E501
        ),
        (
            "dataform",
            "dataform-views",
            "What view types can a dataform have, and who can edit records in each?",
        ),
        (
            "dataform",
            "dataform-limits",
            "What limits or field behaviors are unique to dataforms?",
        ),
        (
            "boards",
            "three-way",
            "I have a request-handling use case: how do I decide between process, board, and dataform?",  # noqa: E501
        ),
    )
    return [
        Query(
...
def _page_queries() -> list[Query]:
    ...
    for label, slug in PAGE_WIDGETS:
        if slug in _VIEW_WIDGETS:
            bind = " showing items from the {flow} process"
        elif slug in _REPORT_WIDGETS:
            bind = " bound to a report over the {flow} process"
        else:
            bind = ""
        out.append(
            Query(
                ...
                expect="Component node + Script.web id + FieldMapping/Property wiring on the page draft",  # noqa: E501
            )
        )
        out.append(
            Query(
                ...
                capability=f"widget.{slug}",
                observable=GRAPH_DIFF,
                prompt=f"On the Probe Dashboard page, configure the {label} widget: set its main options to non-default values.",  # noqa: E501
        ...
        out.append(
            Query(
                ...
                capability=f"widget.{slug}",
                observable=ANSWER,
                prompt=f"When should I use the {label} page widget, and what does it need to work?",  # noqa: E501
    ...
    nav: tuple[tuple[str, str, str], ...] = (
        (
            "nav.create",
            "Create a second navigation set for external users.",
            "new Navigation node under the application draft",
        ),
        (
            "nav.add-menu",
            "Add the Probe Dashboard page to the main navigation menu.",
            "Menu + FieldMapping + Property{Type:Page} chain",
        ),
        (
            "nav.reorder",
            "Reorder the navigation so Probe Dashboard comes first.",
            "Menu order change in Navigation::Menu",
        ),
        (
            "nav.default-page",
            "Make Probe Dashboard the app's default page.",
            "DefaultPage key on the application root",
        ),
        (
            "nav.best-practice",
            "What are the best practices for structuring app navigation across roles?",
            "best-practice row for navigation",
        ),
    )
...
def _role_queries() -> list[Query]:
    ...
    fixed: tuple[tuple[str, str, str, str], ...] = (
        (
            "create",
            "Create an app role called Probe Reviewer.",
            GRAPH_DIFF,
            "new AppRole record (account-level route; engine has no create path — prize capture)",  # noqa: E501
        ),
        (
            "delete",
            "Delete the Probe Reviewer role.",
            GRAPH_DIFF,
            "AppRole removed from account-level list",
        ),
        (
            "assign-user",
            "Add a user to the Probe Reviewer role.",
            GRAPH_DIFF,
            "Members list change on the AppRole detail",
        ),
        (
            "default-page",
            "Set the Probe Reviewer role's default page to Probe Dashboard.",
            GRAPH_DIFF,
            "per-role default page binding on the application draft",
        ),
        (
            "default-navigation",
            "Give the Probe Reviewer role its own navigation.",
            GRAPH_DIFF,
            "per-role Navigation binding",
        ),
        (
            "view-permission",
            "Give Probe Reviewer read-only access to one view of {flow} only.",
            GRAPH_DIFF,
            "view-level permission entry",
        ),
        (
            "report-permission",
            "Share the {flow} report with the Probe Reviewer role.",
            GRAPH_DIFF,
            "report member/batch entry",
        ),
        (
            "tier-semantics",
            "What exactly can a role do at each permission level: No access, Read-only, Edit, Manage?",  # noqa: E501
            ANSWER,
            "best-practice row: the 4-tier permission matrix",
        ),
        (
            "tier-by-module",
            "Do permission levels differ between processes, boards, and dataforms?",
            ANSWER,
            "module-diffs row: 2/4/3-level permission split",
        ),
        (
            "role-best-practice",
            "What roles should a typical approval app define, and with which permissions?",  # noqa: E501
            ANSWER,
            "best-practice row for role design",
        ),
    )
        ...
        for tier in ("No access", "Read-only", "Edit", "Manage"):
            ...
            out.append(
                Query(
                    ...
                    observable=GRAPH_DIFF,
                    prompt=f"Give the Probe Reviewer role {tier} permission on the {{flow}} {module}.",  # noqa: E501
                    expect="permission tier entry for the role on that flow (capture the wire shape per tier)",  # noqa: E501
                )
            )
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--count", action="store_true", help="print per-sweep counts, not JSONL"
    )
