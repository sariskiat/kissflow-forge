"""app.application.intake.questions — the grilling script: what to ask a Thai business
owner to fill in
whichever of the 11 dimensions (schema.py) are still gaps.

Pure and offline: no network, no LLM call, no app-specific vocabulary.
`text_th`/`why`/`example`/
`follow_ups` are written in Thai business language on purpose — the reader is a business
 owner
deciding how their shop should run, never a builder reading wire-level field names.
Every
`example` below illustrates with the SAME neutral, fictional domain (an equipment repair
 intake
shop) so the question set reads as one coherent interview rather than 11 disconnected
prompts;
that domain is illustration only, never baked into `text_th` itself, so the same
question set
reads naturally for a completely different business.

Dimensions 4 (routing), 5 (rework loops) and 7 (master data) are genuinely optional in a
 real
app — a straight-line process with no branch, no rework, and no dropdown list is a
perfectly
normal, complete app, not an incomplete one. Their questions say so explicitly
("ถ้าไม่มีให้ตอบว่า
ไม่มี" — "if there is none, you may answer 'none'"), because `schema.py`'s own
`confirmed_none` flag exists precisely so an interviewer never has to manufacture a fake
 branch or
loop just to satisfy this module — that used to be a real bug here, not a hypothetical
one.
"""
...
from app.application.models.requests.intake.app_spec import AppSpec, _gap_dimensions
...
# The legal FieldType wire values, spelled out so Q6a can tell a business owner exactly
# which
# "ประเภทข้อมูล" (data type) choices actually exist — answering with a type outside this
# list is
...
_Q2: tuple[Question, ...] = (
    Question(
        ...
        why="สิทธิ์การเข้าถึงต้องผูกกับบทบาทจริง ไม่ใช่ผูกกับตัวบุคคลคนเดียว — "
        "และขั้นตอนถัดไปที่จะถามถึง (ใครเป็นเจ้าของแต่ละขั้นตอน) "
        "ต้องอ้างอิงบทบาทที่มีอยู่จริงในลิสต์นี้",
...
_Q6: tuple[Question, ...] = (
    Question(
        ...
        follow_ups=(
            ...
            "ฟอร์มขั้นตอนนี้มีมากกว่าหนึ่งส่วน (section) ไหม เช่น ส่วนข้อมูลทั่วไป "
            "กับส่วนรายละเอียดเฉพาะ?",
        ),
    ),
    Question(
        id="6b",
        ...
        example=(
            ...
            "ช่องคำนวณ 'ราคารวม' = จำนวน x ราคาต่อชิ้น; เลขที่งาน = 'RPR-0001', "
            "'RPR-0002', ..."
        ),
        follow_ups=(
    ...
    Question(
        id="6d",
        text_th="แต่ละส่วน (section) ของฟอร์มที่พูดถึงในข้อ 6a ควรมีคำอธิบายสั้นๆ "
        "ว่าเก็บข้อมูลอะไร — แต่ละส่วนอธิบายว่าอะไร?",
        why=(
...
_Q7: tuple[Question, ...] = (
    Question(
        ...
        text_th=(
            ...
            "มีอะไรบ้าง และใครเป็นคนดูแลรายการนี้? (ถ้าฟอร์มนี้ไม่มี dropdown แบบนี้เลย"
            " ให้ตอบว่า 'ไม่มี' ได้เลย)"
        ),
        why="รายการเหล่านี้ต้องมีค่าที่แน่นอนตายตัว "
        "ไม่งั้นเงื่อนไขที่ผูกกับมันจะพังแบบเงียบๆ — แต่ถ้าฟอร์มใช้แต่ช่องข้อความล้วนๆ"
        " ก็ไม่จำเป็นต้องมีมิติข้อนี้เลย",
        example=(
...
_Q4: tuple[Question, ...] = (
    Question(
        ...
        text_th=(
            ...
            "(ถ้าเป็นงานที่เดินตรงเส้นเดียวไม่มีทางแยกเลย ให้ตอบว่า 'ไม่มี' ได้เลย "
            "ไม่ต้องเสกทางแยกขึ้นมา)"
        ),
        why="ถ้าไม่ระบุจุดแตกแขนง workflow จะเดินเส้นตรงเส้นเดียวเท่านั้น — "
        "และนั่นเป็นคำตอบที่ถูกต้องสมบูรณ์ได้เหมือนกัน ถ้าธุรกิจจริงเป็นแบบนั้น",
        example=(
...
_Q5: tuple[Question, ...] = (
    Question(
        ...
        why="ถ้าไม่ระบุ ระบบจะไม่มีทางส่งงานย้อนกลับให้แก้ไขเลย "
        "งานที่ไม่ผ่านจะค้างอยู่เฉยๆ — แต่ถ้างานจริงไม่เคยต้องย้อนกลับ "
        "ก็ไม่ต้องมีมิติข้อนี้เช่นกัน",
        example=(
            "เช่น ถ้า 'ตรวจสอบคุณภาพ' ไม่ผ่าน ต้องย้อนกลับไปขั้น 'ซ่อม' ใหม่ "
            "จนกว่าจะติ๊กช่อง 'ผ่านคุณภาพแล้ว'"
        ),
        follow_ups=(
            "ตัวชี้วัดว่าออกจากลูปได้แล้วต้องเป็นช่องติ๊กใช่/ไม่ใช่เท่านั้น "
            "ห้ามเป็นตัวเลือกที่เว้นว่างได้ — มีช่องแบบนี้ไหม?",
...
_Q10: tuple[Question, ...] = (
    Question(
        ...
        example=(
            "เช่น หัวหน้าฝ่ายซ่อมบำรุงอยากเห็น: จำนวนงานค้างทั้งหมด, งานที่เกิน SLA, ปุ่มมอบหมายงานใหม่"
        ),
...
# 1 problem/goal comes first (nothing else means anything without it), then 2 roles
# BEFORE
# 3 stages (a stage names an owner_role — asking who owns a stage before any role exists
# puts the
# cart before the horse, a real bug an earlier round of this file had), then 6 data
# model and
# 7 master data BEFORE 4 routing and 5 rework loops (a branch/loop gates on a field, and
# a routing
# literal is checked against a master-data list — both need to exist first), then 8
# visibility,
# then 11 test cases (needs stages/fields/lists to already make sense), then 10
# personas, then
# 9 timing last — it's advisory (see schema.ADVISORY_DIMENSIONS), the most forgiving to
# leave for
# last.
...
def next_questions(spec: AppSpec, limit: int = 4) -> tuple[Question, ...]:
    """The next questions to ask, most-blocking dimension first, capped at `limit`
    total.

    Only dimensions `spec.gaps()` currently flags produce questions — a dimension that's
     already
    answered is never re-asked, and a dimension with no gap never appears here even if
    `limit`
    leaves room. This reads the FULL gap set (`_gap_dimensions`, same as
    `AppSpec.gaps()`), not
    `blocking_gaps()` — dimension 9 (timing) is advisory and never blocks a build, but
    it should
    still get asked about while it's unanswered. Walks `_PRIORITY_ORDER`; within one
    gapped
