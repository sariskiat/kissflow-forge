...
The comparator's axis is spec -> built app, never built app -> reference app: a new
diagram has
no oracle to diff against, so fidelity has to be judged against the input itself (map
#8, check
2). `forge_doctor` stays the referential-integrity check; THIS module is the fidelity
check —
doctor returned `ok` on a build that had wrong field types, a missing event, spurious
Permissions and a misordered table host, because none of those break references (#16
names each
miss; every check below caught a real bug on eval case 1).

Pure and offline: `compare_built_to_spec(draft, spec)` takes the LIVE draft (read back
by the
caller — THE RULE) and the approved `AppSpec`. Every finding lands in exactly one
bucket:
a `mismatches` sentence, a per-rule `checked` count, or a DECLARED `ignored` exclusion
(PublishedBy, the platform's own User node) — known-benign is stated, never silently
skipped.
"""
...
from app.domain.value_objects.field_type import FieldType, trigger_for

Draft = dict[str, Any]

# Node kinds/names the platform itself writes as publish side-effects — declared benign
# (#16:
...
class CompareReport:
    """Every unit examined lands in a bucket: a `mismatches` sentence, a `checked`
    count, or a
    ...
    # `as_tool_result()` (the old dict-with-"isError" shape) is intentionally NOT
    # ported here: `use_cases/intake/forge_compare_to_spec.py` builds
    # `ForgeCompareToSpecResponse` straight from `.ok()`/`.mismatches`/`.checked`/
    # `.ignored` instead -- the whole point of the DTO refactor (spec G10/G12) is
    # that no application-layer object constructs an "isError" dict by hand
    # (scripts/arch_scan.py's `iserror_dicts`, rule 8: new code adds no finding).
...
def _is_unvalidated_select(
    ftype: str, list_name: str | None, node: dict[str, Any]
) -> bool:
...
def _check_field_node(
    ...
    if node.get("Type") != ftype:
        issues.append(
            f"field {name!r} built as {node.get('Type')!r}, input asked {ftype!r}"
        )
...
def _check_sequence(
    ...
    if spec.data_model.sequence is not None:
        ...
        if not any(v.get("Type") == "SequenceNumber" for v in fields.values()):
            problems.append(
                "input asks for an auto-numbered id (sequence) — no SequenceNumber "
                "field was built"
            )
...
def _check_sections(
    ...
    for s in spec.data_model.sections:
        if s.name not in live_sections:
            problems.append(
                f"section {s.name!r} asked for by the input is absent from the form"
            )
...
def _check_empty_banners(ordered: list[dict[str, Any]], problems: list[str]) -> None:
    for i, col in enumerate(ordered):
        if _is_empty_section(col) and _next_item(ordered, i).get("Type") != "Model":
            problems.append(
                f"empty section {col.get('Name')!r} is not immediately followed by a "
                "table "
                f"host — a stranded banner breaks the WHOLE form's render (CLAUDE.md "
                "Tables)"
            )
...
def _check_computed_source(
    ...
    if not got:
        problems.append(
            f"computed {target_field!r}: source {src!r} carries NO event — the value "
            "can never compute"
        )
        return
...
def _unconditional_branches(
    pds: dict[str, dict[str, Any]], branch_ids: list[str]
) -> list[str]:
...
def compare_built_to_spec(draft: Draft, spec: AppSpec) -> CompareReport:
    ...
    return CompareReport(
        mismatches=tuple(problems), checked=checked, ignored=tuple(ignored)
    )
