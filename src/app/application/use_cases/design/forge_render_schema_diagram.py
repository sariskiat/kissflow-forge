"""app.application.use_cases.design.forge_render_schema_diagram -- render the
data-shape draw.io diagram.

Ported from `app.infrastructure.mcp.server.forge_render_schema_diagram` (Stage D
group 8).
"""

from __future__ import annotations

from app.application.exceptions import VERIFY_FAILED, ApplicationError
from app.application.interfaces.artifacts import ArtifactWriter
from app.application.models.requests.design.forge_render_schema_diagram_request import (
    ForgeRenderSchemaDiagramRequest,
)
from app.application.models.responses.design.forge_render_schema_diagram_response import (  # noqa: E501
    ForgeRenderSchemaDiagramResponse,
)
from app.application.use_cases.design._artifacts import content_digest
from app.application.use_cases.design._diagram import schema_diagram_xml


class ForgeRenderSchemaDiagram:
    """Use case behind the `forge_render_schema_diagram` tool."""

    def __init__(self, artifacts: ArtifactWriter) -> None:
        """Build the use case.

        Args:
            artifacts: Writes the rendered diagram to disk.
        """
        self._artifacts = artifacts

    async def execute(
        self, request: ForgeRenderSchemaDiagramRequest
    ) -> ForgeRenderSchemaDiagramResponse:
        """Render the data-shape draw.io diagram and write it to disk.

        Fields grouped by stage, tables with their columns/row cap,
        reference lists with their real values. Same file-writing contract
        and gaps/blocking_gaps echo as `forge_render_flow_diagram`.

        Args:
            request: The validated request.

        Returns:
            The rendered XML, its path on disk, and the spec's gaps.

        Raises:
            ApplicationError: The render itself failed (`code=VERIFY_FAILED`).
        """
        spec = request.spec
        try:
            xml = schema_diagram_xml(spec)
        except (ValueError, TypeError) as exc:
            raise ApplicationError(
                f"failed to render schema diagram: {exc}", code=VERIFY_FAILED
            ) from exc
        path = await self._artifacts.write(
            app_name=spec.app_name,
            digest=content_digest(spec),
            out_dir=request.out_dir,
            filename="schema_diagram.drawio",
            content=xml,
        )
        return ForgeRenderSchemaDiagramResponse(
            xml=xml,
            path=path,
            gaps=list(spec.gaps()),
            blocking_gaps=list(spec.blocking_gaps()),
        )
