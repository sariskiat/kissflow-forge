...
from app.domain.graph import NO_PERMISSION_NODETYPES, ROW_UNITS, section_layout
...
def _check_branch_conditions(
    ...
    branch_exprs = [
        v for v in nodes.values() if v.get("Kind") == "Expression" and v.get("ProcessDef")
    ]
...
def _check_dangling_list_refs(
    ...
    dangling_checked = sum(
        _check_node_dangling_list_refs(nid, node, draft, problems) for nid, node in nodes.items()
    )
...
def _check_user_fields(
    ...
    user_fields = [
        v for v in nodes.values() if v.get("Kind") == "Field" and v.get("Type") == "User"
    ]
...
def _format_list_backed_field_problem(
    ...
    if table is None:
        return (
            head
            + "mint the list with forge_create_list, then re-apply the field with referred_list=<list id>)"
        )
    return (
