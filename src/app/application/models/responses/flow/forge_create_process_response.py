"""Response DTO for `forge_create_process`."""
...
class ForgeCreateProcessResponse(BaseModel):
    """`forge_create_process`'s success result (the former `ProcessCreateReport`).

    Attributes:
        flow_id: The new process's id.
        added: Always empty -- this tool never passes fields.
        skipped: Always empty.
        verified: Always empty.
        missing: Always empty.
        changed_ignored: Always empty.
        collateral: Always empty; kept for shape parity with the former
            `ApplyReport`.
        remediation: Always empty.
