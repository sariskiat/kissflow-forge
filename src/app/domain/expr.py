...
from app.domain.graph import _new_id
...
def expression_owner(node: dict[str, Any]) -> str:
    ...
    present = [
        name
        for key, name in (("ProcessDef", "branch"), ("Activity", "goto"), ("Property", "property"))
        if key in node
    ]
    if len(present) != 1:
        raise ValueError(
    ...
    *,
    ...
    if owner == "goto":
        ...
        if options is not None and literal_value not in options:
            raise ValueError(
                f"literal {literal_value!r} is not one of the field's real options {options}"
            )
