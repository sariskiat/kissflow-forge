...
from app.domain.pages import _instantiate, _mint
from app.domain.types import FieldSpec, FieldType, ParsedField, Visibility
...
def _new_id(kind: str, model_id: str, i: int, name: str) -> str:
    ...
    h = hashlib.sha1(f"{model_id}:{kind}:{i}:{name}".encode(), usedforsecurity=False).hexdigest()[
        :10
    ]
    return f"{kind}_{h}"
...
def clone_template_shell(draft: Draft, template_path: str | None = None) -> Draft:
    ...
    UserTask, and Button::Row. Every node is freshly re-minted (app.domain.pages._instantiate — the
...
def transplant_template(
    ...
    *,
    ...
    is minted fresh, app.domain.pages._mint). Passing the SAME map twice reproduces an IDENTICAL output
    ...
    if not model.get("Model::Appearance"):
        if force_ids:
            ...
            app_id = (
                "Appearance_"
                + hashlib.sha1(f"{basis}:Appearance".encode(), usedforsecurity=False).hexdigest()[
                    :10
                ]
            )
            style_id = (
                "Style_"
                + hashlib.sha1(f"{basis}:Style".encode(), usedforsecurity=False).hexdigest()[:10]
            )
        else:
...
def rename_fields(draft: Draft, renames: dict[str, str]) -> Draft:
    ...
    for old, want in renames.items():
        hits = [
            k
            for k, v in _kind(new, "Field").items()
...
def field_delete_blockers(
    ...
    for nid, node in draft.items():
        ...
        if kind == "Node" and node.get("Type") == "Field" and node.get("Field") in doomed:
            ...
            hit = sorted(fid for fid in dead_fields if isinstance(script, str) and fid in script)
            for fid in hit:
                out.append(
...
def add_table(
    draft: Draft,
    name: str,
    # A mixed list is legal and normal — some columns carry a config dict, most do not. The old
    # `list[2-tuple] | list[3-tuple]` said each list had to be uniform, which no caller is.
    columns: Sequence[
    ...
    if after_section is None:
        ...
        if anchor not in rows:
            raise ValueError(
                f"after_section {after_section!r}: no Section of that name has a root row"
            )
...
def add_sequence_number(
    ...
    if any(
        isinstance(v, dict) and v.get("Type") == "SequenceNumber" and v.get("Name") == field_name
        for v in new.values()
    ):
        return new  # idempotent
...
def _computed_arg_node(
    ...
    if "field" in arg:
        ...
        if ref is None:
            raise ValueError(
                f"set_field_computed({name!r}): no field named {arg['field']!r} to reference"
            )
    ...
    if "static" in arg:
        ...
        return nid
    raise ValueError(
        f"set_field_computed({name!r}): each arg needs one of field/static/fn, got {arg!r}"
    )
...
def set_field_computed(draft: Draft, field_name: str, formula: dict[str, Any]) -> Draft:
    ...
    formula/computed field type" belief; field events (app.domain.graph.set_field_events) remain a
...
def set_conditional_visibility(
    ...
    if not trigger:
        raise ValueError(
            f"set_conditional_visibility: trigger field not found: {trigger_field_name!r}"
        )
    ...
    if not isinstance(trigger_col_id, str) or trigger_col_id not in new:
        raise ValueError(
            f"set_conditional_visibility: trigger {trigger_field_name!r} has no valid Column"
        )
...
def set_field_events(draft: Draft, events: dict[str, list[tuple[str, str]]]) -> Draft:
    ...
    for fname, specs in events.items():
        ...
        for i, (trigger, script) in enumerate(specs):
            if "KFSDK" in script:
                raise ValueError(
                    f"{fname!r}: KFSDK is undefined in a form event — use the injected kf"
                )
...
def _sweep_dangling(draft: Draft) -> None:
    ...
    dangling-ref check uses (src/app/verify.py). A plain key with no "::" holds data, never a node
...
def build_workflow(
    ...
    def _insert_parallel(pname: str, branches: list[Branch]) -> str:
        ...
        for bname, bsteps in branches:
            ...
            new[bpd] = {
                ...
                "Name": bname,
                "Activity": par,
                "ProcessDef::Activity": [_activity(n, "UserTask", bpd, r) for n, r in bsteps],
            }
    ...
    for node in new.values():
        if not (
            isinstance(node, dict) and node.get("Kind") == "Property" and node.get("Name") == "Step"
        ):
            continue
        tgt = node.get("Value")
...
def add_goto_task(
    ...
    *,
    ...
    NO_PERMISSION_NODETYPES): pair with `app.domain.expr.build_goto_gate` to add the Boolean loop
...
def section_layout(draft: Draft) -> SectionLayout:
    ...
    for cid, col in cols.items():
        ...
        is_model_fallback = col.get("Type") == "Model" and name not in section_id_of_name
    ...
    table_hosts = {
        cid for cid, col in cols.items() if col.get("Type") == "Model" or col.get("Column::Model")
    }
...
def _walk_workflow(draft: Draft) -> tuple[dict[str, int], dict[str, str | None]]:
    ...
    if len(roots) != 1:
        raise ValueError(
            f"expected exactly one root ProcessDef (WorkflowType=Sequence), got {len(roots)}"
        )
...
def field_override_matrix(draft: Draft, field_owners: dict[str, list[str]]) -> Matrix:
...
def set_step_permissions(draft: Draft, matrix: Matrix, field_matrix: Matrix | None = None) -> Draft:
    ...
    all_field_cols = (
        {k for k, v in _kind(new, "Column").items() if v.get("Type") == "Field"}
    ...
    if all_field_cols - covered:
        ...
        raise ValueError(
            f"field columns outside every matrix section: {sorted(all_field_cols - covered)}"
        )
