"""Private rule-checking behind `FlowDraft.problems()` — the health-check helpers.

Moved verbatim from the former application-layer health check (G9b):
`doctor()` is renamed `_problems()` and `FlowDraft.problems()`
(`entities/flow_draft.py`) wraps it.
`DoctorReport` keeps its name — it is the one PUBLIC value this module defines,
re-exported by `entities/flow_draft.py` for callers that only want the return
type. Every `_check_*` helper is unchanged; see `entities/flow_draft.py` for
the public docstring and CLAUDE.md for the wire-shape rationale each rule
enforces.

Generic and app-agnostic: no network, no Kissflow calls, no assumptions baked
in about any ONE app's fields, lists or workflow names. Every piece of
app-specific knowledge a rule needs (which list backs a Select field, what its
legal values are) is supplied by the caller through `list_options`. Reports;
never mutates.
"""
...
_PLATFORM_ID_PREFIXES: tuple[str, ...] = (
    "Field_",
    "Column_",
    "Row_",
    "Model_",
...
_PLATFORM_REF_RE = re.compile(
    r"\b((?:" + "|".join(re.escape(p) for p in _PLATFORM_ID_PREFIXES) + r")\w+)\b"
)


_ROUTING_NODE_TYPES: tuple[str, ...] = (*NO_PERMISSION_NODETYPES, "GotoTask")


_LIST_BACKED_FIELD_TYPES: frozenset[str] = frozenset(
    {
        "Select",
        "Multiselect",
        "Checkbox",
...
class DoctorReport:
    """Every check this module runs lands in exactly one bucket, never silently: a
    concrete `problems` sentence, a per-rule `checked` count of units examined, a
    branch literal recorded in `unvalidated` for lack of an options map, or a script
    folded into `unvalidatable_scripts` because its references can never be fully
    proven.
    """
...
def _nodes(draft: NodeGraph) -> dict[str, Any]:
...
def _owning_table_name(nodes: dict[str, Any], field: dict[str, Any]) -> str | None:
    """The NAME of the child table this field belongs to, or None when it is a root form
    field.
    A table is a nested Model hosted by a `Column{Type:"Model"}` (CLAUDE.md > Tables),
    so a child field's `Model` back-ref names the table Model rather than the root. The
    name is read off the table Model and falls back to its HOST column — the two carry
    the same name when this engine writes the table, and a live read-back that drops one
    still resolves the other, the same two-signal discipline `graph._table_model_ids`
    uses.
    Only ever used to pick which REMEDY a problem sentence states, so an unnamed table
    degrades to the root-field remedy rather than a sentence naming `None`.
    """
...
def _validate_root(draft: NodeGraph) -> str:
    root = draft.get("Root")
...
def _check_role_scoped_visibility_claims(
    ...
    for claim in claims:
        problems.append(
            f"{claim}: role-scoped visibility is API-impossible; restructure to "
            f"step-scoped "
...
def _check_branch_children(
    ...
    for _cid, n in children:
        if n.get("Type") == "Field" and n.get("Field") not in draft:
            problems.append(
                f"branch {branch!r} references missing field {n.get('Field')}"
            )
            continue
...
def _check_branch_conditions(
    ...
    branch_exprs = [
        ...
        if v.get("Kind") == "Expression" and v.get("ProcessDef")
    ]
...
def _check_goto_target(
    ...
    if not target:
        ...
        problems.append(
            f"goto {name!r} jumps out of its own branch, to {target.get('Name')!r}"
        )
    elif g["Id"] not in (target.get("Goto::Activity") or []):
        problems.append(
            f"goto {name!r} missing the Goto::Activity back-ref on its target"
        )
...
def _check_goto_expressions(
    ...
    for xid in exprs:
        for root_id in (nodes.get(xid) or {}).get("Expression::Node") or []:
            for cid in (nodes.get(root_id) or {}).get("Node::Node") or []:
                _check_goto_gate_field(
                    name, nodes.get(cid) or {}, draft, nodes, problems
                )
...
def _check_step_stamps(
    ...
    for nid, node in nodes.items():
        if node.get("Kind") == "Property" and node.get("Name") == "Step":
            ...
            if (
                isinstance(tgt, str)
                and (draft.get(tgt) or {}).get("Kind") != "Activity"
            ):
                problems.append(
                    f"sequence Step stamp {nid} -> missing activity {tgt} (publish "
                    f"will 500 MetadataError — repoint it or rebuild via "
                    f"build_workflow, which now repoints by name)"
                )
...
def _check_single_scalar_ref(
    ...
    if ref not in draft:
        problems.append(
            f"{nid}.{key} -> missing {ref} (scalar reference — the list-only sweep "
            f"never "
            f"visits it; PUT 200s, publish dies 500 MetadataError with zero "
            f"diagnostics)"
        )
    return 1, 0
...
def _collect_editable_and_malformed_permissions(
    ...
    susp = {
        v.get("Name")
        for v in nodes.values()
        if v.get("Kind") == "Activity" and v.get("IsSuspended")
    }
    editable: dict[str, set[str]] = {}
...
def _nested_table_model_ids(
    nodes: dict[str, Any], table_host_columns: frozenset[str]
) -> set[str]:
    """Return child table Model ids from both live graph link shapes.

    The usual shape puts child ids in each host Column's ``Column::Model`` list.
    Some read-backs preserve only the child's ``Column`` back-reference, so index
    those references once before walking the hosts. The two passes are linear in
    nodes plus graph edges; they do not scan every Model for every host.
    """
    models_by_host: dict[str, set[str]] = {}
    for model_id, model in nodes.items():
        if model.get("Kind") != "Model":
            continue
        host_id = model.get("Column")
        if isinstance(host_id, str):
            models_by_host.setdefault(host_id, set()).add(model_id)

    nested: set[str] = set()
    for host_id in table_host_columns:
        host = nodes.get(host_id)
        if not isinstance(host, dict):
            continue
        for model_id in host.get("Column::Model") or ():
            if isinstance(model_id, str):
                nested.add(model_id)
        nested.update(models_by_host.get(host_id, ()))
    return nested
...
def _check_editable_sections(
    ...
    sections_checked = 0
    table_models = _nested_table_model_ids(nodes, layout.table_host_columns)
    for node_id, sv in nodes.items():
        if sv.get("Type") in ("Section", "Model") and sv.get("Name"):
            if node_id in layout.table_host_columns or node_id in table_models:
                continue
            sections_checked += 1
            ...
            if not editable.get(sv["Name"]):
                problems.append(
                    f"section {sv['Name']!r} is never editable at any live step"
                )
...
def _check_required_fields(
    ...
    for f in required_fields:
        if not editable.get(layout.owner_section(f.get("Column")) or "?"):
            problems.append(
                f"field {f['Name']!r} is Required but never editable — that step "
...
def _check_permission_matrix(
    ...
    units = (
        {
            ...
            if v.get("Kind") == "Column" and v.get("Type") == "Field"
        }
    ...
    acts = [
        k
        for k, v in nodes.items()
        if v.get("Kind") == "Activity" and v.get("NodeType") not in _ROUTING_NODE_TYPES
    ]
    have = {
        (c, a)
...
def _check_step_assignees(
    ...
    for _aid, v in real_steps:
        ...
        if not _has_app_role_assignee(res_ids, nodes):
            problems.append(
                f"step {v.get('Name')!r} has no AppRole assignee — submit will 500 "
                f"(CLAUDE.md "
                f"Members first: wire a Resource with ValueType 'AppRole' on every "
                f"step)"
            )
...
def _check_user_fields(
    ...
    user_fields = [
        v
        for v in nodes.values()
        if v.get("Kind") == "Field" and v.get("Type") == "User"
    ]
    checked["user_fields"] = len(user_fields)
    for f in user_fields:
        qids = f.get("Field::QueryDefinition") or []
        has_query_def = any(
            (nodes.get(q) or {}).get("Kind") == "QueryDefinition" for q in qids
        )
        if not has_query_def:
            problems.append(
                f"User field {f.get('Name')!r} has no QueryDefinition sibling — "
                f"publish will fail (KISSFLOW_ERROR_04211, CLAUDE.md #59)"
            )
...
def _format_list_backed_field_problem(
    ...
    name = f.get("Name")
    head = (
        f"{f.get('Type')} field {name!r} has no ReferredList — a dropdown bound "
        f"to no list; PUT 200s and publish dies 500 MetadataError with zero "
        f"diagnostics (CLAUDE.md ReferredList wiring #13 — "
    )
    if table is None:
        return (
            head + "mint the list with forge_create_list, then re-apply the field "
            "with referred_list=<list id>)"
        )
    return (
...
def _check_list_backed_fields(
    ...
    list_backed = [
        v
        for v in nodes.values()
        if v.get("Kind") == "Field" and v.get("Type") in _LIST_BACKED_FIELD_TYPES
    ]
...
def _check_placeholder_names(
    ...
    for f in todo_names:
        ...
        if isinstance(name, str) and _TODO_MARKER in name:
            problems.append(
                f"field name {name!r} carries a shipped TODO placeholder — a developer "
                f"note is "
                f"being published as a user-facing label "
                f"(shapes/process_template_identity_shell.json; move the note out of "
                f"Name)"
            )
...
def _check_column_span_validity(
    ...
    if not isinstance(start, int) or not isinstance(end, int):
        problems.append(
            f"column {label!r} has no numeric grid span (Start={start!r}, End={end!r}) "
            f"— the "
    ...
    if not (0 <= start < end <= ROW_UNITS):
        problems.append(
            f"column {label!r} sits at (Start={start}, End={end}), off the "
            f"{ROW_UNITS}-unit "
            f"row grid (0 <= Start < End <= {ROW_UNITS}) — the builder fails to render "
            f"the "
            f"whole flow"
        )
        return False
        ...
        for i, (s1, e1, n1) in enumerate(spans):
            for s2, e2, n2 in spans[i + 1 :]:
                if s2 >= e1:
                    break
                problems.append(
                    f"columns {n1!r} ({s1}, {e1}) and {n2!r} ({s2}, {e2}) overlap in "
                    f"row {rid} — "
    ...
    for rid, row in nodes.items():
        ...
        for cid in row.get("Row::Column") or []:
            ...
            if isinstance(own, str) and own != rid:
                problems.append(
                    f"column {cid} is listed in row {rid} but its own Row back-ref "
                    f"names {own} — "
                    f"two rows claim the same column, so one row renders it and the "
                    f"other renders "
                    f"a hole"
                )
...
def _problems(
    ...
    *,
    ...
    `list_options` maps a Kissflow LIST id (the value of a Select field's
    `ReferredList`) to its live legal option values. A branch's literal is checked
    against real options ONLY when its sibling comparison is against a Select field
    whose list id is a key of this map; every other literal is recorded in `unvalidated`
    — never silently accepted, never silently flagged.
    `visibility_role_claims` carries the SPEC's role-scoped visibility claims (one
    human-readable sentence each), supplied by the caller because the graph itself can
    never hold one — a Permission node is (column, step), never (column, role), so a
    role-scoped claim is API-impossible and each one FAILs the audit outright with a
    stated reason (#6, ADR-0004: refuse, never best-effort). Compile threads these from
    `VisibilityEntry.role` into the plan's doctor op; a caller with no spec in hand just
    leaves it empty.
    """
    ...
    _check_branch_conditions(
        draft, nodes, resolved_options, problems, unvalidated, checked
    )
    ...
    editable, malformed_permissions = _collect_editable_and_malformed_permissions(
        nodes, layout
    )
    _check_editable_sections(nodes, layout, editable, problems, checked)
