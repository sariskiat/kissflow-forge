...
class FieldType(StrEnum):
    ...
    DATE = "Date"
    DATETIME = "DateTime"  # calendar date + time-of-day, ONE node type. Captured live via the
    # copilot field sweep #45 (2026-08-12): shapes/field_datetime.json,
    # docs/capabilities/field.date-time.md. The enum predated that
    # capture; it is not a guess.
    BOOLEAN = "Boolean"
    SELECT = "Select"
    USER = "User"
...
class EventTrigger(StrEnum):
    ...
    Lives here, not in `app.application.intake.schema` where it was born, because BOTH the spec layer
    ...
    `onClick` on the other. `app.application.intake.schema` re-exports it, so every existing import site
...
def trigger_for(t: FieldType) -> EventTrigger:
    ...
    match t:
        ...
        case _:
            # Reached today by FieldType.DATETIME, which no live capture has ever paired with a
            # trigger. Until this match covered every member it fell through and returned None
            # implicitly, so callers doing `trigger_for(t).value` got an opaque AttributeError
            # instead of a reason. Naming the gap beats inventing a trigger for it: DATETIME is
            # deliberately absent from TRIGGER_LIVE_CONFIRMED, and inferring onSelect from the
            # Date family is the guess CLAUDE.md THE RULE exists to stop.
            raise ValueError(
                f"no live capture pairs the {t.value!r} field type with an event trigger. "
                f"This engine refuses the field. It does not guess a trigger from a similar "
                f"field type, because a guessed trigger never fires (issue #12)"
            )
