...
Knowledge stays data, this module only clones and wires it, mirroring app.domain.graph's convention:
...
# rand>"); unlike app.domain.graph's deterministic field ids (which must be idempotent-reconcile-safe
...
def _apply_style_props(style_node: dict, props: dict[str, Any]) -> None:
    ...
    same convention as app.domain.graph.set_section_style).
    """
...
def _style_pairs_to_props(style: Any) -> dict[str, Any]:
    ...
    app.application.intake.schema.DesignNode.style) into a Style.Value props dict for `add_container`'s
...
def build_design(page: Draft, *, parent_id: str, design: dict[str, Any]) -> tuple[Draft, list[str]]:
    ...
    app.application.intake.schema.DesignNode) as a child of `parent_id`. Pure (returns a NEW draft).
    ...
    new, host_id = add_widget(
        page, container_id=parent_id, widget=widget["slug"], config=config, name=name or None
    )
...
def _bind_repeater_row_label(
    draft: Draft, *, page_id: str, content_id: str, field: str
) -> tuple[Draft, str]:
...
def add_widget(
    ...
    if row_fields:
        ...
        if content_id is None:
            raise ValueError(
                f"widget {widget!r} has no row-template container to bind row_fields into"
            )
        vref_ids: list[str] = []
...
def add_event_mapping(
    ...
    *,
    ...
    if type not in _EVENT_ARM_NODES:
        raise ValueError(
            f"unknown event mapping type {type!r}; known types: {sorted(_EVENT_ARM_NODES)}"
        )
