...
DEV-ONLY by default: app.infrastructure.kissflow.client reads KF_DEV_* and refuses any domain without "dev-", unless
KF_DOMAIN is set instead — an explicit opt-in to any tenant (see app.infrastructure.kissflow.client._tenant_env).
...
Three tool families:
  ...
  forge_* the P2 surface (Node G): thin wrappers around app.infrastructure.kissflow.client / app.infrastructure.kissflow.pages_live /
          app.infrastructure.kissflow.dataplane's live orchestration functions. Every forge_* tool body is ONE call
          ...
          reimplement it).
  forge_* the P3 surface (Node K, bottom of the file): app.application.intake (grill questions, AppSpec,
          compile-to-BuildPlan) + app.application.design (draw.io diagrams, HTML mockups, the confirmation
          ...
          (app.application.intake.serde), since the server itself holds no session state. forge_plan_app
          ...
          (app.infrastructure.mcp.server._mint_approval_token, keyed by a per-process secret generated once at
...
from app.application.compare import compare_built_to_spec
...
from app.application.intake.questions import QUESTIONS, next_questions
from app.application.intake.schema import DIMENSION_NAMES, AppSpec, blank_spec
...
from app.infrastructure.kissflow.auth import creds_from_token
from app.infrastructure.kissflow.auth import provider_from_env as auth_provider_from_env
...
from app.infrastructure.kissflow.dataplane import LiveDataPlane, StepPlan, field_name_index, walk
...
from app.infrastructure.playbook import load_playbook

# HTTP mode authenticates through Entra and then binds the caller's own Kissflow pair from the
# OAuth client fields (see app.infrastructure.kissflow.auth). `provider_from_env` fails closed for incomplete config;
...
def _client(app_id: str | None = None, require_app: bool = True) -> KfClient | Err:
    ...
    if pair is not None:
        ...
        return Err(
            "config",
            "not authenticated — complete Entra login with your own Kissflow "
...
def _result(x: Any) -> dict[str, Any]:
    ...
    in app.infrastructure.kissflow.client/app.infrastructure.kissflow.pages_live and every Err carries `.as_tool_result()`; a few
...
# DataKind / DeleteKind are defined in client.py, next to the functions whose contract they
# ARE (apply_fields / delete_anything), and imported above. They stay a single definition so
# the tool schema and the function signature cannot drift apart.
...
def kf_get_flow_schema(
    flow_kind: SchemaKind, flow_id: str, app_id: str | None = None
) -> dict[str, Any]:
    ...
    got = c.get_draft(flow_kind, flow_id)
    return got.as_tool_result() if isinstance(got, Err) else got
...
def kf_apply_field_change(
    ...
    try:
        ...
        return Err("verify", str(e)).as_tool_result()
    return _result(apply_fields(c, flow_kind, flow_id, specs, publish=publish))
...
def kf_create_process(
    ...
    try:
        ...
        return Err("verify", str(e)).as_tool_result()
    return _result(
        create_process(c, name, tuple(steps), specs, publish=publish, from_template=from_template)
    )
...
def kf_set_step_visibility(
    ...
    include_pairs: Annotated[
        bool,
        Field(
            description="Return every (column, activity) permission pair in full, resolved to field/section/step names, instead of the bounded counts + by_section/by_step rollups. The default already lists every `missing` pair in full and states how many entries it summarised."
        ),
    ] = False,
    app_id: str | None = None,
) -> dict[str, Any]:
    ...
    try:
        ...
        return Err("verify", str(e)).as_tool_result()
    return apply_step_permissions(
        c, flow_id, matrix, publish=publish, include_pairs=include_pairs
    ).as_tool_result()
...
# =====================================================================================
# forge_* — P2 surface (Node G). Thin wrappers; logic lives in app.infrastructure.kissflow.client /
# app.infrastructure.kissflow.pages_live / app.infrastructure.kissflow.dataplane. Every write tool is dev-only, KF_APP-scoped (KfConfig
...
def forge_create_process(
    name: str, publish: bool = False, from_template: bool = True, app_id: str | None = None
) -> dict[str, Any]:
    ...
    if isinstance(c, Err):
        return c.as_tool_result()
    return _result(
        create_process(c, name, ("Draft",), [], publish=publish, from_template=from_template)
    )
...
def forge_apply_fields(
    ...
    return _result(
        apply_fields_full(
            ...
            computed=computed,
            conditional=conditional_visibility,
            publish=publish,
        )
    )
...
def forge_apply_layout(
    ...
    try:
        ...
        return Err("verify", str(e)).as_tool_result()
    return _result(
        apply_layout(c, flow_id, typed, descriptions=descriptions, publish=publish, kind=kind)
    )
...
def forge_set_branch_conditions(
    ...
    if isinstance(c, Err):
        return c.as_tool_result()
    return _result(
        apply_branch_conditions(c, flow_id, field_name, branch_literals, publish=publish, kind=kind)
    )
...
def forge_set_events(
    ...
    **Pass `null` for the trigger and it is DERIVED from the source field's live type**, which is
    ...
    Geolocation (`app.domain.types.NO_EVENT_FIELD_TYPES`, which this refusal reads directly). CLAUDE.md
...
def forge_delete_fields(
    ...
    return _result(
        delete_fields(
            c, flow_id, tuple(fields or ()), tuple(tables or ()), publish=publish, kind=kind
        )
    )
...
def forge_publish(kind: PublishKind, flow_id: str, app_id: str | None = None) -> dict[str, Any]:
    ...
    if isinstance(detail, Err):
        return {
            ...
            "status": None,
            "isError": True,
            "error": f"publish succeeded but status read-back failed: "
...
def forge_doctor(
    ...
    if isinstance(c, Err):
        return c.as_tool_result()
    return _result(run_doctor(c, flow_id, kind=kind, visibility_role_claims=visibility_role_claims))
...
def forge_build_page(
    ...
    app.domain.pages builder. A widget whose binding is load-bearing (view/*, report/*, metrics,
...
def forge_set_navigation(
    ...
    if isinstance(c, Err):
        return c.as_tool_result()
    return _result(
        apply_navigation(c, app_id, page_id, label, unify=unify, sweep=sweep, publish=publish)
    )
...
def forge_simulate_case(
    ...
    try:
        ...
        return Err("verify", str(e)).as_tool_result()
    plans = [
        StepPlan(name=s["name"], values=s["values"], reject=s["reject"], comment=s["comment"])
        for s in parsed
    ]
...
def forge_list_apps() -> dict[str, Any]:
    ...
    if not isinstance(got, list):
        return Err(
            "http", f"list_applications returned an unexpected shape: {got!r}"
        ).as_tool_result()
...
def forge_delete_flow(kind: DeleteKind, flow_id: str, app_id: str | None = None) -> dict[str, Any]:
...
# logic lives in app.infrastructure.kissflow.client (or app.infrastructure.capabilities for the offline docs tool); every write
    ...
    groups: Annotated[
        ...
        Field(
            description=(
                ...
                "confirm_group_notification=True, because granting a group EMAILS every member."
            )
        ),
    ] = None,
    confirm_group_notification: Annotated[
        ...
        Field(
            description=(
                ...
                "developer with `user_query` when testing."
            )
        ),
    ] = False,
    force_regrant_groups: Annotated[
        ...
        Field(
            description=(
                ...
                "Still requires confirm_group_notification=True in the same call."
            )
        ),
    ] = False,
    app_id: str | None = None,
...
def forge_copilot_check(
    ...
    if isinstance(c, Err):
        return c.as_tool_result()
    return _result(
        apply_copilot_check(c, app_id, conversation_id, baseline_inventory=baseline_inventory)
    )


# =====================================================================================
# forge_* — P3 surface (Node K): app.application.intake (grill questions, AppSpec, compile) +
# app.application.design (draw.io diagrams, HTML mockups, confirmation protocol) exposed over MCP, STATELESS —
# the spec is a plain dict in and out of every call (app.application.intake.serde), never held here
...
# content. A plain content digest (app.application.design.spec_digest, freely computable by anyone from
...
# Question id -> dimension number (1..11), derived from app.application.intake.questions.QUESTIONS (a
...
# the one piece forge_intake_questions needs that nothing in app.application.intake exposes directly.
...
def _content_digest(spec: AppSpec) -> str:
    ...
    content with `approved` normalized to False before hashing (app.application.design.spec_digest).
...
def forge_update_spec(spec: dict[str, Any] | None, patch: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(patch, dict):
        return Err(
            "verify", f"patch must be an object, got {type(patch).__name__}"
        ).as_tool_result()
...
def forge_request_confirmation(spec: dict[str, Any], out_dir: str | None = None) -> dict[str, Any]:
    ...
    paths = {
        name: _write_artifact(directory, name, content) for name, content in req.artifacts.items()
    }
    return {
...
def forge_approve_spec(
    spec: dict[str, Any], digest: str, decision: ApprovalDecision
) -> dict[str, Any]:
