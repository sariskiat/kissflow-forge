...
from app.domain.expr import build_branch_condition, build_goto_gate, remove_condition
...
Draft = dict[str, Any]
# Three DIFFERENT closed sets hide under the parameter name `kind`, and collapsing them is a
# real API bug, not a tidy-up: tests/test_mcp_boundary.py::test_the_different_kind_sets_are_NOT_
# unified pins that apart, because `kf_publish(flow_kind=...)` exposes FlowKind as a PUBLIC MCP
# schema and publishing a word list is meaningless.
#
#   FlowKind    — publishable and graph-bearing. The narrowest, and the one MCP tools expose.
#   DataKind    — has a field-bearing draft graph. Adds "dataset": field writes there produce the
#                 identical node shapes and are live-proven (docs/capabilities/module.dataform.md,
#                 #50). A word list is excluded — it has no draft graph to write fields into.
#   AnyFlowKind — anything addressable at a /flow or /metadata URL. `kind` is interpolated into
#                 the path segment and only branched on for `== "process"`, so the read/list/
#                 create/delete methods genuinely take all five (see _SWEEP_FLOW_KINDS).
#
# These three replace eleven `# type: ignore[arg-type]` comments that were papering over the
# single narrow alias.
...
class KfConfig:
    ...
    @staticmethod
    def from_user(
        key_id: str, key_secret: str, app_id_override: str | None = None
    ) -> KfConfig | Err:
        """Same config, but the ACCESS-KEY PAIR comes from the calling user (app.infrastructure.kissflow.auth carries
...
class KfClient:
    ...
    # --- endpoints -------------------------------------------------------
    def _draft_url(self, kind: AnyFlowKind, flow_id: str) -> str:
        c = self._cfg
        return f"{c.base}/metadata/2/{c.account}/{kind}/{flow_id}/draft?_application_id={c.app_id}"

    def get_draft(self, kind: AnyFlowKind, flow_id: str) -> Draft | Err:
        return self._json("GET", self._draft_url(kind, flow_id))

    def create_flow(self, kind: AnyFlowKind, name: str) -> str | Err:
        c = self._cfg
        got = self._json(
            "POST", f"{c.base}/flow/2/{c.account}/{kind}?_application_id={c.app_id}", {"Name": name}
        )
    ...
    def archive_flow(self, kind: AnyFlowKind, flow_id: str) -> None | Err:
        """A process MUST be archived before it can be deleted (400 KISSFLOW_ERROR_04602 otherwise)."""
        c = self._cfg
        got = self._json(
            "POST",
            f"{c.base}/flow/2/{c.account}/{kind}/{flow_id}/archive?_application_id={c.app_id}",
        )
        return got if isinstance(got, Err) else None

    def delete_flow(
        self, kind: AnyFlowKind, flow_id: str, archive_first: bool = True
    ) -> None | Err:
        c = self._cfg
        if archive_first and kind == "process":
            ...
            if isinstance(archived, Err):
                return archived
        got = self._json(
            "DELETE", f"{c.base}/flow/2/{c.account}/{kind}/{flow_id}?_application_id={c.app_id}"
        )
        return got if isinstance(got, Err) else None
    ...
    def publish(self, kind: FlowKind, flow_id: str) -> None | Err:
        ...
        got = self._json(
            "POST",
            f"{c.base}/metadata/2/{c.account}/{kind}/{flow_id}/publish?_application_id={c.app_id}",
        )
        return got if isinstance(got, Err) else None
    ...
    def get_flow_detail(self, kind: FlowKind, flow_id: str) -> dict[str, Any] | Err:
        ...
        c = self._cfg
        return self._json(
            "GET", f"{c.base}/flow/2/{c.account}/{kind}/{flow_id}?_application_id={c.app_id}"
        )

    def list_flows(self, kind: AnyFlowKind) -> list[dict[str, Any]] | Err:
        ...
        c = self._cfg
        return self._json(
            "GET", f"{c.base}/flow/2/{c.account}/{kind}?_application_id={c.app_id}&page_size=100"
        )

    def get_members(self, kind: FlowKind, flow_id: str) -> list[dict[str, Any]] | Err:
        c = self._cfg
        return self._json(
            "GET", f"{c.base}/flow/2/{c.account}/{kind}/{flow_id}/member?_application_id={c.app_id}"
        )
    ...
    def list_app_roles(self, app_id: str | None = None) -> list[dict[str, Any]] | Err:
        ...
        while True:
            got = self._json(
                "GET", f"{c.base}/app_role/2/{c.account}/list?page_number={page}&page_size=100"
            )
            if isinstance(got, Err):
                return got
    ...
    def put_app_role(
        self, role_id: str, body: dict[str, Any], app_id: str | None = None
    ) -> Any | Err:
        ...
        return self._json(
            "PUT", f"{c.base}/app_role/2/{c.account}/{role_id}?_application_id={scope}", body
        )
    ...
    def get_assignee(self, query: str) -> list[dict[str, Any]] | Err:
        ...
        # driven in Thai (app.application.intake.questions is a Thai interview script), so a non-ASCII
        ...
        return self._json(
            "GET", f"{c.base}/user/2/{c.account}/assignee?q={urllib.parse.quote(query, safe='')}"
        )
    ...
    def post_member_batch(
        self, kind: FlowKind, flow_id: str, members: list[dict[str, Any]]
    ) -> Any | Err:
        ...
        return self._json(
            "POST",
            f"{c.base}/flow/2/{c.account}/{kind}/{flow_id}/member/batch?_application_id={c.app_id}",
            members,
        )
    ...
    def create_app_role(self, name: str, app_id: str | None = None) -> str | Err:
        ...
        scope = app_id if app_id is not None else c.app_id
        got = self._json(
            "POST", f"{c.base}/app_role/2/{c.account}", {"Name": name, "_application_id": scope}
        )
        if isinstance(got, Err):
            return got
    ...
    def get_list_items(self, list_id: str) -> list[str] | Err:
        ...
        c = self._cfg
        return self._json(
            "GET", f"{c.base}/flow/2/{c.account}/list/{list_id}/items?_application_id={c.app_id}"
        )
    ...
    def list_lists(self) -> list[dict[str, Any]] | Err:
        ...
        c = self._cfg
        return self._json(
            "GET", f"{c.base}/flow/2/{c.account}/list?page_size=100&_application_id={c.app_id}"
        )
    ...
    def create_list(self, name: str) -> dict[str, Any] | Err:
        ...
        c = self._cfg
        return self._json(
            "POST", f"{c.base}/flow/2/{c.account}/list?_application_id={c.app_id}", {"Name": name}
        )
    ...
    def set_list_items(self, list_id: str, items: list[str]) -> Any | Err:
        ...
        c = self._cfg
        return self._json(
            "POST", f"{c.base}/flow/2/{c.account}/list/{list_id}/items", {"ListItems": items}
        )
    ...
    def create_dataset(self, name: str) -> dict[str, Any] | Err:
        ...
        return self._json(
            "POST",
            f"{c.base}/flow/2/{c.account}/dataset?_application_id={c.app_id}",
            {"Name": name},
        )
    ...
    def create_case(self, name: str, item_type: str, prefix: str) -> dict[str, Any] | Err:
        ...
        return self._json(
            "POST",
            f"{c.base}/flow/2/{c.account}/case?_application_id={c.app_id}",
    ...
    def create_dataset_record(self, flow_id: str, record: dict[str, Any]) -> dict[str, Any] | Err:
        ...
        c = self._cfg
        return self._json(
            "POST", f"{c.base}/dataset/2/{c.account}/{flow_id}?_application_id={c.app_id}", record
        )
    ...
    def list_dataset_records(self, flow_id: str) -> dict[str, Any] | Err:
        ...
        c = self._cfg
        return self._json(
            "GET", f"{c.base}/dataset/2/{c.account}/{flow_id}/list?_application_id={c.app_id}"
        )

    def update_dataset_record(
        self, flow_id: str, record_id: str, record: dict[str, Any]
    ) -> dict[str, Any] | Err:
        ...
        return self._json(
            "PUT",
            f"{c.base}/dataset/2/{c.account}/{flow_id}?_application_id={c.app_id}&_id={record_id}",
            record,
        )

    def delete_dataset_record(
        self, flow_id: str, record_id: str, name: str
    ) -> dict[str, Any] | Err:
    ...
    def list_pages(self, app_id: str) -> list[dict[str, Any]] | Err:
        ...
        c = self._cfg
        return self._json(
            "GET", f"{c.base}/flow/2/{c.account}/application/{app_id}/page?page_size=100"
        )

    def create_page(self, app_id: str, name: str) -> str | Err:
        c = self._cfg
        got = self._json(
            "POST", f"{c.base}/flow/2/{c.account}/application/{app_id}/page", {"Name": name}
        )
        if isinstance(got, Err):
            return got
        page_id = got.get("_id")
    ...
    def delete_page(self, app_id: str, page_id: str) -> None | Err:
        ...
        c = self._cfg
        got = self._json(
            "DELETE", f"{c.base}/flow/2/{c.account}/application/{app_id}/page/{page_id}"
        )
        return got if isinstance(got, Err) else None
    ...
    def put_page_draft(
        ...
        return self._read_verify_write(
            lambda: self.get_page_draft(app_id, page_id),
            self._page_draft_url(app_id, page_id),
            new,
            expect_version,
        )
    ...
    def put_draft(
        self, kind: DataKind, flow_id: str, new: Draft, expect_version: str | None
    ) -> Draft | Err:
        """Read-verify-write. Re-reads the live version and aborts if it drifted since we planned."""
        return self._read_verify_write(
            lambda: self.get_draft(kind, flow_id),
            self._draft_url(kind, flow_id),
            new,
            expect_version,
        )
...
def scaffold_inventory(draft: Draft) -> ScaffoldInventory:
    ...
    sections = tuple(
        sorted(
            n
            for v in _kind(draft, "Column").values()
            if v.get("Type") == "Section" and isinstance(n := v.get("Name"), str) and n
        )
    )
    required = tuple(
        sorted(n for n, node in _root_field_nodes(draft).items() if n and node.get("Required"))
    )
    steps = tuple(
        sorted(
            n
            for a in _kind(draft, "Activity").values()
            if a.get("NodeType") not in NO_PERMISSION_NODETYPES
            and isinstance(n := a.get("Name"), str)
            and n
        )
    )
...
def create_process(
    ...
    def _abandon(err: Err) -> Err:
        client.delete_flow("process", flow_id)
        return err
    ...
    written = client.put_draft(
        "process", flow_id, scaffolded, expect_version=draft.get(_META_VERSION)
    )
    ...
    return ProcessCreateReport(
        ...
        changed_ignored=report.changed_ignored,
...
def _permission_nodes(draft: Draft) -> list[tuple[str, dict[str, Any]]]:
    ...
    return [
        (k, v) for k, v in draft.items() if isinstance(v, dict) and v.get("Kind") == "Permission"
    ]
...
def _malformed_permissions(draft: Draft) -> tuple[str, ...]:
    ...
    return tuple(
        sorted(
            k
            for k, n in _permission_nodes(draft)
...
def _section_field_names(draft: Draft, section_name: str) -> tuple[str, ...]:
    ...
    name_of_col = {
        ...
        if isinstance(n, dict) and n.get("Kind") == "Field" and isinstance(n.get("Column"), str)
    }
    sid = next(
        (
            k
...
def _field_placements(draft: Draft) -> dict[str, tuple[str, int, int, int]]:
    ...
    def _leaves(
        row_ids: list[str], title: str, ri: int, out: dict[str, tuple[str, int, int, int]]
    ) -> None:
        for rid in row_ids:
            for cid in (draft.get(rid) or {}).get("Row::Column") or []:
    ...
    for sec in draft.values():
        if not (
            isinstance(sec, dict) and sec.get("Kind") == "Column" and sec.get("Type") == "Section"
        ):
            continue
        title = sec.get("Name", "")
...
def _layout_collateral(before: Draft, after: Draft, exclude: Iterable[str] = ()) -> tuple[str, ...]:
    ...
    for name in sorted(was):
        ...
        if name not in now:
            out.append(
                f"{name!r} was laid out in section {was[name][0]!r} and is now in no "
...
def _pair_names(draft: Draft) -> _PairNames:
    ...
    step_of_activity = {
        aid: n for aid, a in _kind(draft, "Activity").items() if isinstance(n := a.get("Name"), str)
    }
    return _PairNames(field_of_column, section_of_column, step_of_activity)


def _uncovered_sections(
    draft: Draft, matrix: Matrix, field_matrix: Matrix | None
) -> tuple[str, ...]:
...
class StepPermissionReport(ApplyReport):
    ...
    def as_tool_result(self) -> dict[str, Any]:
        ...
        if self.include_pairs:
            out["pairs"] = {
                "added": list(self.added_named),
                "skipped": list(self.skipped_named),
                "verified": list(self.verified_named),
                "missing": list(self.missing_named),
            }
            ...
            out["note"] = (
                ...
                if not self.missing
                else f"{summarised} written/skipped/verified/collateral entries summarised into "
...
def apply_fields(
    ...
    if publish and not (missing or ignored.entries):
        # A born-live kind (dataset, list) has no publish route at all — calling it returns a
        # bare 404 from the tenant. The docstrings said "leave publish False"; nothing enforced it.
        # Refuse here so the caller gets the reason instead of an HTTP status.
        if kind not in ("form", "process", "case"):
            return Err(
                "verify",
                f"{kind!r} has no publish route. Its draft is already live. A publish "
                f"request returns HTTP 404. Call this tool again with publish=False.",
            )
        pub = client.publish(kind, flow_id)
        if isinstance(pub, Err):
            return pub
...
# app.domain.graph/expr/verify function -> guarded PUT -> READ-BACK verify -> optional publish.
...
def _sync_fields_and_layout(
    ...
    written = _write_fields_and_layout_draft(client, kind, flow_id, draft, new, added, groups)
    if isinstance(written, Err):
        return written
    return client.get_draft(kind, flow_id)
...
def apply_fields_and_layout(
    ...
    read_back = _sync_fields_and_layout(client, kind, flow_id, draft, new, added, groups)
    if isinstance(read_back, Err):
        return read_back
...
def apply_layout(
    ...
    return ApplyReport(
        flow_id=flow_id,
        added=(),
        skipped=wanted_sections,
        verified=wanted_sections,
        missing=(),
        changed_ignored=(),
        collateral=collateral,
...
class TableReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "verified_columns": list(self.verified_columns),
            "missing_columns": list(self.missing_columns),
            "meta_version": self.meta_version,
            "published": self.published,
            "isError": bool(self.missing_columns),
        }
...
class ListReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "isError": bool(self.missing_items),
        }
...
def apply_workflow(
    ...
    return WorkflowReport(
        ...
        permissions_deleted=max(permissions_deleted, 0),
...
class GotoGateReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            "flow_id": self.flow_id,
            "goto_activity_id": self.goto_activity_id,
...
def _parallel_branches(draft: Draft) -> dict[str, str] | Err:
    ...
    parallels = [
        ...
        if isinstance(v, dict) and v.get("Kind") == "Activity" and v.get("NodeType") == "Parallel"
    ]
    if len(parallels) != 1:
        return Err(
            "verify", f"expected exactly one Parallel gateway on the flow, found {len(parallels)}"
        )
    names: list[str] = []
    out: dict[str, str] = {}
...
def _resolve_activity_by_name(
    ...
    *,
    ...
    if len(candidates) > 1:
        owners = sorted(
            {_describe_owner(draft, draft.get(c, {}).get("ProcessDef")) for c in candidates}
        )
        return Err(
            "verify",
...
def apply_goto_gate(
    ...
    if branch_name is not None:
        ...
        if branch_pd_id is None:
            return Err(
                "verify",
                f"no branch named {branch_name!r} on the Parallel gateway "
                f"(real branches: {sorted(branches)})",
            )
        target_id = _resolve_activity_by_name(
            draft, target_activity_name, process_def_id=branch_pd_id
        )
    else:
    ...
    try:
        with_goto, goto_id = add_goto_task(
            draft, target_activity_id=target_id, branch_process_def_id=branch_pd_id
        )
    ...
    return GotoGateReport(
        flow_id=flow_id,
        goto_activity_id=goto_id if verified else None,
...
def _literals_for_field(draft: Draft, pd_id: str, field_id: str) -> set[str]:
    ...
    for eid in pd.get("ProcessDef::Expression") or []:
        ...
        for root_id in e.get("Expression::Node") or []:
            children = [
                draft.get(c) or {} for c in (draft.get(root_id) or {}).get("Node::Node") or []
            ]
...
def apply_branch_conditions(
    ...
    if unknown:
        return Err(
            "verify",
            f"no branch(es) named {unknown} on the Parallel gateway "
            f"(real branches: {sorted(branches)})",
        )
    ...
    try:
        for branch_name, literal in branch_literals.items():
            ...
            new = build_branch_condition(
                new, process_def_id=pd_id, field_id=field_id, literal=literal, options=options
            )
    except ValueError as e:
    ...
    graph = read_back  # narrowed here; the closure below cannot see the isinstance above

    def _landed(branch_name: str, literal: str) -> bool:
        pd = graph.get(branches[branch_name]) or {}
        for eid in pd.get("ProcessDef::Expression") or []:
            e = graph.get(eid) or {}
            for root_id in e.get("Expression::Node") or []:
                children = [
                    graph.get(c) or {} for c in (graph.get(root_id) or {}).get("Node::Node") or []
                ]
                has_field = any(
                    c.get("Type") == "Field" and c.get("Field") == field_id for c in children
                )
                has_literal = any(
                    c.get("Type") == "Static" and c.get("Value") == literal for c in children
                )
...
def _resolve_event_triggers(
    ...
    """
    by_name = {
        v.get("Name"): v for v in draft.values() if isinstance(v, dict) and v.get("Kind") == "Field"
    }
    ...
    for fname, specs in events.items():
        ...
        if node is None:
            ...
            if any(t in (None, "") for t, _s in specs):
                raise ValueError(
                    f"cannot derive a trigger for {fname!r}: no field of that name is on this flow"
                )
    ...
    return _EventPlan(
        resolved, tuple(triggers), tuple(dict.fromkeys(derived)), tuple(dict.fromkeys(unverified))
    )
...
def apply_section_style(
    ...
    try:
        new = set_section_style(
            draft, styles, root_style=root_style, hint_text_position=hint_text_position
        )
    except ValueError as e:
    ...
    def _landed(name: str) -> bool:
        ...
        wanted = styles[name]
        return all(
            value.get(k) == _style_wire_value(k, v) for k, v in wanted.items() if v is not None
        )
    ...
    def _root_landed() -> bool:
        ...
        ok = all(
            value.get(k) == _style_wire_value(k, v)
...
def run_doctor(
    ...
    try:
        report = doctor(
            draft, list_options=list_options, visibility_role_claims=visibility_role_claims or ()
        )
    except ValueError as e:
...
class MemberReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "roles_granted": len(self.applied),
...
def _apply_own_app_roles(
    client: KfClient, target_flow_id: str, kind: FlowKind
) -> MemberReport | Err:
    ...
    # Off `usable`, not off `members`: the posted payload mixes str with list[str] (Permission),
    # so reading an id back out of it is a `str | list[str]`, and MemberReport wants plain str.
    ...
    return MemberReport(
        ...
        + (f"; {len(unusable)} seen but NOT granted: {', '.join(unusable)}" if unusable else "")
        + (
            "; a role you created and do not see counted here was not on the account list "
            "when this ran — re-run forge_member_batch"
            if not unusable
            else ""
        ),
    )
...
def apply_member_roles(
    ...
    for r in existing:
        if not isinstance(r, dict):
            continue
        _name, _rid = r.get("Name"), r.get("_id")
        if isinstance(_name, str) and isinstance(_rid, str):
            by_name[_name] = _rid
    ...
    role_ids = tuple(resolved)  # already dict[str, str]; see the note at the site above
    names = tuple(resolved.values())
    ...
    return MemberReport(
        ...
        missing=missing,
        role_ids=role_ids,
        resolved=tuple((name, rid) for rid, name in resolved.items()),
        note=note,
    )
...
def create_application_verified(client: KfClient, name: str) -> dict[str, Any] | Err:
    ...
    return {
        "app_id": app_id if verified else None,
        "name": name,
        "verified": verified,
        "supported": True,
        "note": None,
...
def delete_anything(
    ...
    if kind == "page":
        if not app_id:
            return {
                ...
                "verified": False,
                "isError": True,
                "error": "app_id is required to delete a page",
            }
        ...
        if isinstance(deleted, Err):
            return {
                ...
                "verified": False,
                "isError": True,
                "error": deleted.as_tool_result()["error"],
            }
        ...
        if isinstance(listed, Err):
            return {
                ...
                "verified": False,
                "isError": True,
                "error": listed.as_tool_result()["error"],
            }
    ...
    # process | form | case
    deleted = client.delete_flow(kind, flow_id, archive_first=True)
    if isinstance(deleted, Err):
        return {
            "kind": kind,
            "id": flow_id,
...
def _execute_and_verify_role_update(
    ...
    body = _build_role_write_body(
        prep.detail,
        prep.existing_members,
        prep.new_ones,
        prep.existing_groups,
        prep.new_groups,
    )
...
def apply_grant_tier(
    ...
    if by_tier is None:
        return Err(
            "verify", f"forge_grant_tier: unknown kind {kind!r} — valid: {sorted(_TIER_MAP)}"
        )
    if tier not in by_tier:
        return Err(
            "verify",
            f"forge_grant_tier: unknown tier {tier!r} for kind {kind!r} — valid: {sorted(by_tier)}",
        )
...
def create_flow_any(
    ...
    extra = extra or {}
    if kind == "form":
        fid = client.create_flow(kind, name)
    ...
    if kind == "process":
        ...
        written = client.put_draft(
            "process", fid, scaffolded, expect_version=draft.get(_META_VERSION)
        )
        if isinstance(written, Err):
            client.delete_flow("process", fid)
            return written
    ...
    return Err(
        "verify",
        f"create_flow_any: unknown kind {kind!r} — valid: process, form, list, dataset, case",
    )
...
def publish_application_verified(client: KfClient, app_id: str) -> dict[str, Any] | Err:
    ...
    return {
        ...
        "meta_version": draft.get(_META_VERSION),
        "isError": False,
        "note": None
        if runtime_id
        else (
...
def create_template_app(client: KfClient, name: str) -> dict[str, Any] | Err:
    ...
    def _abandon(err: Err) -> Err:
        ...
        if isinstance(role_id, str):
            role_gone = app.delete_app_role(
                role_id
            )  # best-effort, same convention as create_process's cleanup
    ...
    if isinstance(detail, Err):
        return _abandon(
            Err("verify", f"publish succeeded but status read-back failed: {detail.message}")
        )
...
def _resolve_dataset_record_keys(
    ...
    from app.infrastructure.kissflow.dataplane import field_name_index, resolve_value_keys

    draft = client.get_draft("dataset", flow_id)
    if isinstance(draft, Err):
        return Err(
            "verify",
            f"cannot resolve dataform field names — draft read failed: {draft.message}",
            status=draft.status,
        )
...
class DatasetRecordClient(Protocol):
    """The four-action slice of KfClient that `apply_dataset_records` actually drives.

    Same reasoning as dataplane.ItemDetailReader: the function needs four dataform record
    operations, not the whole client, and asking for the whole client is what stops a test
    supplying a focused in-memory fake. `KfClient` satisfies this structurally.
    """

    def create_dataset_record(
        self, flow_id: str, record: dict[str, Any]
    ) -> dict[str, Any] | Err: ...
    def list_dataset_records(self, flow_id: str) -> dict[str, Any] | Err: ...
    def update_dataset_record(
        self, flow_id: str, record_id: str, record: dict[str, Any]
    ...
    # `_resolve_dataset_record_keys` reads the dataform draft to map field NAMES to ids.
    def get_draft(self, kind: AnyFlowKind, flow_id: str) -> Draft | Err: ...
...
def apply_dataset_records(
    ...
    if op == "update":
        ...
        if not record_id:
            return Err(
                "verify", "apply_dataset_records(op='update') requires record_id (the record's _id)"
            )
        resolved = _resolve_dataset_record_keys(client, flow_id, record)
        if isinstance(resolved, Err):
            return resolved
    ...
    if op == "delete":
        if not record_id:
            return Err(
                "verify", "apply_dataset_records(op='delete') requires record_id (the record's _id)"
            )
        name = (record or {}).get("Name")
        if not name:
            return Err(
                "verify",
                "apply_dataset_records(op='delete') requires record={'Name': "
    ...
    return Err(
        "verify", f"apply_dataset_records: unknown op {op!r} — valid: create, list, update, delete"
    )
...
class RolePreferenceReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "default_navigation": self.default_navigation,
            "verified": self.verified,
            "isError": not self.verified,
        }
...
def apply_set_role_preference(
    ...
    verified = all(
        live_pref.get(k) == v
        for k, v in (("DefaultPage", default_page), ("DefaultNavigation", default_navigation))
        if v is not None
    )
    return RolePreferenceReport(
        role_id=role_id,
...
def run_sweep(client: KfClient, scope: str, app_id: str | None = None) -> dict[str, Any]:
    ...
    for s in wanted:
        if s == "apps":
            ...
            by_kind = {kind: _sweep_bucket(client.list_flows(kind)) for kind in _SWEEP_FLOW_KINDS}
...
def apply_copilot_ask(
    ...
    if isinstance(convs, Err):
        ...
        status = (
            "matched"
            if conversation_id
            else "pending: your message is not in the thread yet — this is NORMAL, the copilot "
...
def _flow_id_inventory(client: KfClient) -> dict[AnyFlowKind, list[str]] | Err:
...
class CopilotCheckReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "landed_nodes": self.landed_nodes,
...
def apply_copilot_check(
    ...
    if isinstance(convs, Err):
        return convs
    thread = [
        c for c in convs if isinstance(c, dict) and c.get("ConversationId") == conversation_id
    ]
    ...
    for kind, ids in current.items():
        ...
        counts: dict[str, int] = {}
        for fid in new_ids:
            draft = client.get_draft(kind, fid)
...
class FullFieldsReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "isError": bool(
                self.missing
                or self.changed_ignored
                or self.validations_missing
                or self.computed_missing
...
def apply_fields_full(
    ...
    (`app.application.tools._to_spec` folds a `default_value` key into `options["DefaultValue"]`), since
    ...
    try:
        ...
        for fname, cond in conditional.items():
            new = set_conditional_visibility(
                new, fname, cond["trigger_field"], cond["operator"], cond["rhs"]
            )
    ...
    for fname, rules in validations.items():
        ...
        for rule in rules:
            ...
            (val_verified if (rule["operator"], rule["rhs"]) in live_rules else val_missing).append(
                key
            )
    ...
    return FullFieldsReport(
        ...
        collateral=collateral,
        remediation=ignored.remediation + (("forge_apply_layout",) if collateral else ()),
        validations_verified=tuple(val_verified),
        validations_missing=tuple(val_missing),
        computed_verified=computed_verified,
        computed_missing=computed_missing,
        conditional_verified=tuple(cond_verified),
        conditional_missing=tuple(cond_missing),
        meta_version=read_back.get(_META_VERSION),
        published=published,
    )
...
def delete_fields(
    ...
    try:
        ...
        if blockers:
            return Err(
                "verify",
                "refusing to delete — these references would be left dangling: "
                + "; ".join(blockers),
            )
    ...
    collateral = tuple(
        sorted(
            ...
            for kind_name, n in Counter(
                (draft.get(nid) or {}).get("Kind", "?") for nid in doomed
            ).items()
        )
    )
...
class RenameFieldsReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "meta_version": self.meta_version,
            "published": self.published,
            "isError": bool(self.missing or self.stale),
        }
...
def rename_form_fields(
    ...
    if clashes:
        return Err(
            "verify",
            f"refusing to rename onto name(s) already on this form: {clashes} — "
    ...
    unchanged = tuple(f"{old} -> {n}" for old, n in renames.items() if old == n and n in live_after)
    real = {old: n for old, n in renames.items() if old != n}
    verified = tuple(
        f"{old} -> {n}" for old, n in real.items() if n in live_after and old not in live_after
    )
    missing = tuple(f"{old} -> {n}" for old, n in renames.items() if n not in live_after)
    stale = tuple(
        f"{old} -> {n}" for old, n in real.items() if n in live_after and old in live_after
    )
...
def _cleared_required_fields(
    ...
    cleared = [
        name for name, node in root_fields.items() if node.get("Required") and name not in wanted
    ]
...
def _prepare_required_draft(
    ...
    if unsatisfiable:
        return Err(
            "verify",
            f"refusing to mark un-fillable field(s) Required: {unsatisfiable} — "
