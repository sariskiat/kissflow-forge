...
from app.infrastructure.kissflow.client import Err, KfClient, KfConfig, _tenant_env
...
def _open(kind: str, token: str, ttl_s: int | None) -> dict[str, Any] | None:
    ...
    try:
        ...
        raw = (
            ...
            if ttl_s is None
            else fernet.decrypt(token.encode(), ttl=ttl_s)
        )
...
def _validate_pair(key_id: str, key_secret: str) -> bool:
    ...
    cfg = KfConfig(key_id=key_id, key_secret=key_secret, account=account, domain=domain, app_id="")
...
class KissflowOAuthProvider(AzureProvider):
    ...
    async def load_authorization_code(
        ...
        _register_token_cleanup(lambda: self._release_authorization_code_claim(authorization_code))
        return code
