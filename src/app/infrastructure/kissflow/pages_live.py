...
Mirrors app.infrastructure.kissflow.client's own convention exactly (GET -> apply an offline builder -> guarded PUT ->
READ-BACK verify -> optional publish), kept in its OWN file for the same reason app.domain.pages and
app.domain.nav are split out from app.domain.graph: a page/application draft is a DIFFERENT node-graph
...
`app.infrastructure.kissflow.client.KfClient` (already carrying the page/app HTTP methods) with `app.domain.pages` /
`app.domain.nav`'s pure builders — it holds no HTTP logic of its own.
"""
...
from app.domain.nav import add_page_menu, point_all_navs_at, sweep_orphans
...
from app.infrastructure.kissflow.client import Err, KfClient
...
def create_page_flow(
    ...
    return PageReport(
        app_id=app_id,
        page_id=page_id if verified else None,
        name=name,
        verified=verified,
        published=published,
    )
...
class PageBuildStep:
    ...
    "bind" | "design"; `kwargs` are passed straight to the matching app.domain.pages builder
...
def apply_page_build(
    ...
    """GET the page draft -> apply every step offline via app.domain.pages's own builders (each
    ...
    Every app.domain.pages builder already fails loud (ValueError) on a placeholder/unbound widget
    ...
    try:
        for step in steps:
            if step.kind == "container":
                ...
                host = step.kwargs[
                    "host"
                ]  # bind_widget() below requires it; fail there, not on a None label
...
class NavigationReport:
    ...
    def as_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "swept_orphans": list(self.swept_orphans),
            "meta_version": self.meta_version,
            "published": self.published,
            "isError": self.menu_id is None,
        }
...
def apply_navigation(
    ...
    if not navs:
        return Err(
            "verify", f"application {app_id!r} draft has no Navigation node to attach a menu to"
        )
    nav_id = navs[0]
    ...
    try:
        ...
        if len(added_ids) != 1:
            return Err(
                "verify", f"expected exactly 1 new Menu id, got {len(added_ids)}: {added_ids}"
            )
        menu_id = added_ids[0]
    ...
    return NavigationReport(
        app_id=app_id,
        menu_id=verified_id,
        unified_nav_ids=tuple(navs) if unify else (),
        swept_orphans=swept,
        meta_version=read_back.get(_META_VERSION),
        published=published,
    )
...
def _build_popup_widgets(
    state: _PageOpState, pname: str, container_id: str | None, widgets: Any
) -> None:
    # container_id is None only when the popup came back with no Popup::Container at all (see the
    # `or [None]` at the call site). That is harmless while there is nothing to place, and
    # unbuildable the moment there is — so say which, rather than passing None down to add_widget.
    if not widgets:
        return
    if container_id is None:
        raise ValueError(f"the popup {pname!r} has no Popup::Container node to hold its widgets")
    for w in widgets:
...
def _wire_action_event(state: _PageOpState, action: str, host: str, event: dict[str, Any]) -> None:
    kind = event.get("kind")
    try:
        if kind == "OpenPopup":
...
def _is_unknown_popup_target(event: dict[str, Any] | None, popup_ids: dict[str, str]) -> bool:
    if event and event.get("kind") == "OpenPopup":
        return event.get("target_popup") not in popup_ids
    return False


def _build_single_action(state: _PageOpState, action: str, event: dict[str, Any] | None) -> None:
